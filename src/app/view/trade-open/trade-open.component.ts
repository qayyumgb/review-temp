...
@Component({
  selector: 'app-trade-open',
  templateUrl: './trade-open.component.html',
  styleUrl: './trade-open.component.scss'
})

export class TradeOpenComponent implements AfterViewInit, OnInit, OnDestroy{
  sectorData: any = [];
  indexData: any = [];
  indSector: any = [];
  ...
  constructor(private datepipe: DatePipe, private dialogref: MatDialogRef<TradeOpenComponent>, public dialog: MatDialog, public indexService: IndexstepService,
  ...
  clickData(datas:any, item:any, type:any) {
    ...
    if (type == 'DI') {
      ...
      indexId = (isNotNullOrUndefined(selIndex['indexId'])) ? selIndex['indexId'] : 123;
  ...
  GBMHPtrade: any;
  GPerETFRList: any;
  factsheetError: boolean = false;
  openClickedTradeAccount(datas: any, refresh: boolean = false) {
    $('.refreshBt span').addClass('rotate');
    var that = this;
    that.factsheetError = false;
    that.bldMyIndexMntHPortfolioData = [];
    that.GetETFStrData = [];
    that.showSpinner_trade = true;
    try {
      try { this.GTtrade.unsubscribe(); } catch (e) { }
      this.GTtrade = this.dataService.GetTaxharvesttrade(datas).pipe(first()).subscribe((res: any) => {
        if (this.sharedData.checkServiceError(res)) {
          ...
          that.update_perfasofDate(res);
              ...
              if (isNotNullOrUndefined(this.clickedList['taxEffAwareness']) && this.clickedList['taxEffAwareness'] == "Y") { checktaxeff = true; }
          if (res.length > 2 && checktaxeff) {
             this.taxOptFactSheet = true; 
            }
          ...
          this.GBMHPtrade = this.dataService.GetBuildMyIndexMnthlyHPtrade(datas).pipe(first()).subscribe((res: any) => {
            $('.refreshBt span').removeClass('rotate');
            ...
            if (this.sharedData.checkServiceError(res)) {
              ...
              res = [...res].sort(function (x, y) { return descending((parseFloat(x.weight)), (parseFloat(y.weight))); });
        ...
        //$('#myCustomIndErrorModal_review').modal('show');
        that.closecommon_progress();
        that.factsheetError = true;
        setTimeout(function () { that.showSpinner_trade = false; }, 1000);
      });
  ...
  refreshMonthlyTrade() {
    ...
    that.bldMyIndexMntHPortfolioData = [];
    that.showSpinner_trade = true;
    this.GBMHPtrade = this.dataService.GetBuildMyIndexMnthlyHPtrade(this.dMyIndexMnthlyHPtradePayload).pipe(first()).subscribe((res: any) => {
      $('.refreshBt span').removeClass('rotate');
      that.toastr.success('Updated successfully', '', { timeOut: 4000 });
      if (this.sharedData.checkServiceError(res)) {
        $('#myCustomIndErrorModal_review').modal('show');
        that.closecommon_progress();
        that.logger.logError(res, 'GetBuildMyIndexMnthlyHPtrade');
      } else {
        res = [...res].sort(function (x, y) { return descending((parseFloat(x.weight)), (parseFloat(y.weight))); });
        that.bldMyIndexMntHPortfolioData = res;
        /*var dta: any = res;*/
    ...
    if (Data.length > 0) {
      ...
      this.perfData.years.forEach(d => {
        ...
        if (that.taxOptFactSheet) {
          ...
          //console.log(this.taxOptFactSheet,'this.taxOptFactSheet 2')
        }
      });
  ...
  indexStep() {
    ...
    that.dataService.GetStrategyNotifyId(currentList.id, currentList.accountId).pipe(first()).subscribe((nID: any) => {
      this.notifyId = nID;
      if (isNotNullOrUndefined(this.notifyId) && this.notifyId.length > 0) {
        if (this.sharedData.selTradeData.type == "CI") {
          this.indexService.GetUserSteps(this.notifyId[0]['notifyId'], this.sharedData.selTradeData.type).then((res: any) => {
  ...
  closecommon_progress() {
    ...
    if (this.bldMyIndexMntHPortfolioData.length > 0 && this.GetETFStrData.length > 0) {
      setTimeout(function () { that.showSpinner_trade = false; }.bind(this), 1000);
      //setTimeout(function () { 
      //  if (that.perfData.years.length > 0){
      //    // if ($(".TradecalendarYearReturns").scrollLeft() + $(".TradecalendarYearReturns").innerWidth() >= ($(".TradecalendarYearReturns")[0].scrollWidth) - 1) {
      //    //   $(".TradecalendarYearReturns").addClass('widthExt1');
      //    //   console.log($(".TradecalendarYearReturns").scrollLeft() + $(".TradecalendarYearReturns").innerWidth() >= ($(".TradecalendarYearReturns")[0].scrollWidth) - 1,'width')
      //    //   $("#TradeArrowWrap").hide();
      //    // }else{
      //    //   $(".TradecalendarYearReturns").removeClass('widthExt1');
      //    // }
      //  }
      ////    if ($(".TradetaxGainLoss").scrollLeft() + $(".TradetaxGainLoss").innerWidth() >= ($(".TradetaxGainLoss")[0].scrollWidth) - 1) {
      ////   $(".TradetaxGainLoss").addClass('widthExt1');
      ////   $("#TradetableNavBuy2").hide();
      //// };
      //}, 100);
  ...
  hideNav() {
    setTimeout(function () {
      $(".TradecalendarYearReturns").scrollLeft(0);
      $(".TradetaxGainLoss").scrollLeft(0);
      if ($(".TradecalendarYearReturns").length > 0 && $(".TradecalendarYearReturns").scrollLeft() + $(".TradecalendarYearReturns").innerWidth() >= ($(".TradecalendarYearReturns")[0].scrollWidth) - 1) {
        $(".TradecalendarYearReturns").addClass('widthExt1');
        $("#TradeArrowWrap2").hide();
      }else{
        $(".TradecalendarYearReturns").removeClass('widthExt1');
        $("#TradeArrowWrap2").show();
      }

      if ($(".TradetaxGainLoss").length> 0 && Math.round($(".TradetaxGainLoss").scrollLeft())  + $(".TradetaxGainLoss").innerWidth() >= ($(".TradetaxGainLoss")[0].scrollWidth) - 1) {
        $(".TradetaxGainLoss").addClass('widthExtTax');
        $("#TradetableNavBuy2").hide();
      }else{
        $(".TradetaxGainLoss").removeClass('widthExtTax');
        $("#TradetableNavBuy2").show();
      }
     }, 50)
 }
  ...
  onScroll(event: Event,params:any) {
    ...
    const isAtEnd = Math.round(scrollableElement.scrollLeft) + Math.round(scrollableElement.offsetWidth) + 1  >= scrollableElement.scrollWidth;
    if (isAtEnd) {
  ...
  onScrolltaxGainLoss(event: Event,params:any) {
    ...
    if (isAtEnd) {
        ...
        // console.log(isAtEnd,element,'isAtEnd')
      if (element) {
        element.classList.add('scrollBtn');
      }
    } else {
      const element = document.getElementById('taxRightArrow');
      // console.log(isAtEnd,element,'else')
      if (element) {
        element.classList.remove('scrollBtn');
      }
    }
  ...
  MK: boolean = false;
  convert2digits(val:any, id:any) {if (isNullOrUndefined(val)) {return 0;  } else { if (id == 17){this.MK = true;return this.billionconverter(val);} else{  this.MK = false; return parseFloat(val).toFixed(1); };}  }
  ...
  checkconst_filters() {
    var that = this;
    if (that.indexRulesData[0].strStockkey.length > 0 || that.indexRulesData[0].strSector.length > 0 || that.accord_losers_length > 0 || that.factorsGrp.length > 0) {
      return true;
    } else { return false; }
  }
  ...
  createSector(data:any) {
    ...
    list1.forEach((x: any) => {
      ...
      let tot = sum(list, function (d: any) { return d.Wt; });
      arr.push({ "name": x, weight: format(".2f")(tot) + "%" });
    });
    var dta = [...arr].map(a => ({ ...a })).sort(function (x, y) { return descending((parseFloat(x.weight)), (parseFloat(y.weight))); });
    return [...dta];
  }

  ETFIndex(index:any) {
    var that = this;
    var ETFIndex: any = [...that.sharedData._ETFIndex].filter(x => x.assetId == index.assetId);
    if (ETFIndex.length > 0) {
      that.cusIndexService.GetAllocScoresFactMC(index['assetId'],ETFIndex[0]['holdingsdate']).then((res: any) => { this.loadChartInfo(res); });     
    } else {
      var indID = this.cusIndexService.checkMyIndexId(index.assetId);
      if (index.assetId == 55551111 || index.assetId == 55553333) {
        this.cusIndexService.GetAllocScoresFactMC(indID, this.sharedData.equityHoldDate).then((resDt: any) => { this.loadChartInfo(resDt); });
      } else { this.cusIndexService.GetEquityScores(indID, this.sharedData.equityHoldDate).then((resDt: any) => { this.loadChartInfo(resDt); }); }
    }
  }

  loadChartInfo(data: any) {
    var that = this;
    ...
    var dta = that.indexData.map((a: any) => ({ ...a })).sort(function (x: any, y: any) { return descending((parseFloat(x.Wt)), (parseFloat(y.Wt))); });
  ...
  barChart() {
    ...
    //  var flDta1 = that.sectorData.filter((y: any) => y.name == x);
    ...
    Highcharts.chart({
      chart: {
        renderTo: 'barChartBuyModel_popup_Trade',
        type: 'bar',
  ...
  format(d:any) { return format(",.2f")(d) }
    ...
    if (Data.length > 0) {
      Data.forEach((x: any, i: any) => {
        if (isNotNullOrUndefined(x['top100']) && isNotNullOrUndefined(x['range']) && x['range'] != "" && x['top100'] != "") {
          ...
          if (that.taxOptFactSheet) { 
            afterTax.push(that.GetETFStrData[2]['graphvalues'][i]['range']);
           }
        }
      });
    }
  ...
  getBarchart(payload:any) {
    ...
    var name1: String = (isNotNullOrUndefined(this.currentAccountInfo['shortname'])) ? this.currentAccountInfo['shortname'] : '';
    var accountid: number = (isNotNullOrUndefined(payload['accountid'])) ? payload['accountid'] : 0;
    if (this.checkIndexTab == 'DI') { name1 = this.clickedList.name; };
    var data: any = {
      "name1": name1,
      "name2": (isNotNullOrUndefined(this.currentAccountInfo['Indexname'])) ? this.currentAccountInfo['Indexname'] : '',
      "accountid": accountid,
      "assetid": assetid,
      ...
      "notifyid": (isNotNullOrUndefined(this.notifyId) && this.notifyId.length > 0) ? this.notifyId[0]['notifyId'] : 0,
    ...
    if (this.checkIndexTab == 'DI' && accountid == 0) {
      var DI: any = [...this.dirIndexService.getBetaIndex_Direct.value].filter((x: any) => x.assetId == assetid);
      if (DI.length > 0) {
        var data: any = { "id": DI[0]['id'], "enddate": data['enddate'], "indexType": "", "userid": userid };
        var etfHoldDate = [...this.sharedData._ETFIndex].filter(x => x.assetId == assetid);
        if (etfHoldDate.length > 0) { data.indexType = "ETF"; }
       this.dataService.DIfactsheetSectordataAPI(data).pipe(first()).subscribe((res: any) => {
          var chartData = (isNotNullOrUndefined(res['dIFactsheetSectorlistdata'])) ? [...res['dIFactsheetSectorlistdata']] : [];
          chartData = chartData.map((item: any) => ({
            indexName: item.name,
            bmName: (isNotNullOrUndefined(this.currentAccountInfo['Indexname'])) ? this.currentAccountInfo['Indexname'] : '',
            ...item
          }));
          this.barchartData = [...chartData];
          try { this.barChart(); } catch (e) { }
        }, (error: any) => {
          this.barchartData = [];
          try { this.barChart(); } catch (e) { }
        })       
  ...
  checkWeightflags(d:any) {
    if (isNotNullOrUndefined(d) && d.length > 0 && isNotNullOrUndefined(d[0]['strData']) &&
      isNotNullOrUndefined(d[0]['strData']['weightlimitflag']) && isNotNullOrUndefined(d[0]['strData']['weightlimit']) &&
      d[0]['strData']['weightlimitflag'] == 'Y') { return true } else { return false }
  }
