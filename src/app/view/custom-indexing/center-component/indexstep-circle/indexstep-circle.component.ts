...
@Component({
  selector: 'app-indexstep-circle',
  templateUrl: './indexstep-circle.component.html',
  styleUrl: './indexstep-circle.component.scss'
})
export class IndexstepCircleComponent {
  ...
  constructor(public sharedData: SharedDataService, public cusIndexService: CustomIndexService, public preIndexService: PrebuildIndexService, private indexStepSer: IndexstepService,) { }
  ngOnInit() {
    var that = this;
    this.sharedData.showCenterLoader.next(true);    
  
    var unSubHdata = this.cusIndexService.performanceTenYrETFRList.subscribe(data => { this.getTradeDate(data); });

    this.cusIndexService.getExclCompData_indexsteps().then((d:any) => {
      var data: any = d['res'];
      data = [...data];
      this.total_components = data.length;
      var clkd_index = that.cusIndexService.indexStepsGridData.value;
      //that.showCircleScores = false;
      if (data.length > 0) {
        data = this.checkNewSort(data);
        //that.centercircleData = data.map(a => ({ ...a }));
        this.loadData(data);
      } else {
    ...
    //this.creatGradienArc('#excluCircle_main_steps');
   
    this.subscriptions.add(unSubHdata);
    var selCompany = this.cusIndexService.selCompany.subscribe(res => {
      if (isNotNullOrUndefined(res)) {
        this.checkHandlePos(res);
      } else { d3.select("#excluCircle_main_steps").selectAll("#cSlider").remove(); };
  ...
  checkHandlePos(d: any) {
    var exclCompData = [...this.excompData].map(a => ({ ...a }));
    ...
    try {
      ...
      var filExcDt = this.cusIndexService.exclusionCompData.value.filter((x: any) => x.stockKey == d.stockKey);
      if (filExcDt.length>0) {
        this.impRevValue = d3.format(".1f")(filExcDt[0]['impRev'] * 100);
        this.STW_Pricecurrency = filExcDt[0]['currency'] + ' ' + d3.format(",.1f")(filExcDt[0]['price']);
      }
    } catch (e) { console.log(e) }
  }
  getTradeDate(res:any) {
    var that = this;
    that.tradeDt = '';
    that.sinceDt = '';
    var xx = res;
    if (xx.length > 0) {
      var since = xx[0].sinceDate;
  ...
  creatClockSlider(selComp: any, rank: any) {
    let that = this;
    d3.select("#excluCircle_main_steps").selectAll("#cSlider").remove();
    if (isNotNullOrUndefined(selComp) && isNotNullOrUndefined(selComp.isin)) {
      ...
      var g = d3.select("#excluCircle_main_steps").select("#crlChartSteps").append("g").attr("id", "cSlider").attr('transform', 'rotate(' + (-90) + ')')
  ...
  SetClockHandle(d: any, rank: any) {
    let that = this;
    let txt = d.compname;
    var txt1 = "(" + d.ticker + ") " + d3.format(".1f")(d.scores * 100) + "%";
    if (that.resetCircleScore()) {
      txt1 = "(" + d.ticker + ") " + d3.format(".1f")(d.scores * 100) + "%";
    } else { txt1 = "(" + d.ticker + ") "; }
    if ((d.compname.length) > 18) { txt = d.compname.slice(0, 18).trim() + "..."; }

    let val = d.cx;
    var sliTag = d3.select("#excluCircle_main_steps").select("#cSlider");
    var r: any = this.radius;
    ...
    if (d.compname.length > 25) { midtxt = d.compname.slice(0, 20) + "... (" + d.ticker + ") "; }
    that.selCompText = midtxt;
    that.HFscore = d3.format(".1f")(d.scores * 100) + '%';

      var medVel: any = d3.format(".1f")(d.scores);
      if (that.resetCircleScore()) {
        var per = d3.scaleLinear().domain([0, that.excompData.length]).range([0, 100]);
        var colorVal = per(rank - 1);
        d3.select("#excluCircle_main_steps").select('.Sel_midCompS1').select('.SelCompScore').attr('text-anchor', 'middle').style("fill", function () { return that.cl(colorVal); })
        ...
        d3.select("#excluCircle_main_steps").select('.Sel_midCompS1').select('.SelCompScore').attr('text-anchor', 'middle').style("fill", function () { return that.cl(medVel + 1); })
          .style('font-size', ((that.sharedData._uname.indexOf('technogradient.com') > -1) || (that.sharedData._uname.indexOf('newagealpha.com') > -1)) ? '40px' : '48px')
          .style('font-family', "var(--ff-bold)").text(medVel);
        sliTag.select('.sRect1').style("fill", function () {
  ...
  resetCircleScore() {
    let that = this;
    var notifyId = [...that.sharedData.getNotificationQueue.value].filter(x => x.slid == this.cusIndexService._currentSList.id && x.accountId == 0);
    if (notifyId.length == 0) { notifyId = [...that.sharedData.getNotificationQueue.value].filter(x => x.slid == this.cusIndexService._currentSList.id.id) };
    if (notifyId.length > 0 && isNotNullOrUndefined(notifyId[0]['selectBy']) && isNotNullOrUndefined(notifyId[0]['weightBy'])) {
      if (notifyId[0]['selectBy'] == 1 || notifyId[0]['weightBy'] == 1) { return true; } else { return false; };
    } else { return false; };
  }
  ...
  creatGradienArc(masterID: string) {
    ...
    gArc = d3.select(masterID).select("#crlChartSteps").append("g").attr("id", "gradNewArc");
    gArcLine = d3.select(masterID).select("#crlChartSteps").append("g").attr("id", "gradArcLine");
  ...
  loadData(res:any) {
    ...
    if (res.length > 0) {
      ...
      this.creatGradienArc('#excluCircle_main_steps');
      this.CreateComps('#excluCircle_main_steps', [...res], "lvl1");
  ...
  CreateComps(masterID: string, data: any, lvl: string) {
    let that = this;
    var compLst;
    var dta = this.checkNewSort([...data]);
    var oSvg = d3.select(masterID).select("#crlChartSteps");
    ...
    //      } else { return 'comp'; };
    //    } catch (e) { console.log(e); return 'comp'; }
    ...
    //  });
    var compg = compC.selectAll("g").data(dta).enter().append("g")
      .attr("class", function (d, i) { return 'comp'; })
      .attr("transform", function (d, i) { return "rotate(" + (d.cx) + ")"; })
      //.attr("name", function (d) { return d.isin + "_" + d.indexName.replace(/ /g, '_') })
      .on("mouseover", function (d) {
        d3.select(this).raise();
        //if (that.breadcrumbdata.length > 0) { d3.select("#gCircleSlider").style("display", "block"); }
      })
      .style("display", function (d) { return "block"; })
    ...
    /** Close Loader **/
    setTimeout(() => { this.sharedData.showCenterLoader.next(false); }, 1000)
    /** Close Loader **/
  }
  ...
  fillCompetives(dta: any) {
    let that = this;
    var gs = null;
    var data = this.checkNewSort([...dta]);
    d3.select("#excluCircle_main_steps #crlChartSteps").select("#gCompetitive").remove();
    d3.select("#excluCircle_main_steps #crlChartSteps").select("#exclugCompetitiveRect").remove();
    gs = d3.select("#excluCircle_main_steps").select('#crlChartSteps').append("g").attr("id", "gCompetitive");
    d3.select("#excluCircle_main_steps").select('#crlChartSteps').append("g").attr("id", "exclugCompetitiveRect");
    ...
    var ggs = gs.selectAll("g")
      ...
      .style("opacity", function (d) {
        ...
        else if (data.length > 180) { opa = 1 - (((sMax - sMin) / 100) - (.6)); }
        return opa;
      });
    ...
    ggs.on("pointerenter", function (event, d) {
      if (data.length > fillMaxlen) {
        d3.select("#excluCircle_main_steps #exclugCompetitiveRect").raise();
        d3.select("#excluCircle_main_steps").select("#cSlider").raise();
      } else {
    ...
    if (data.length > fillMaxlen) {
      that.fillCompetivesRect(data);
    } else {
      d3.select("#excluCircle_main_steps #exclugCompetitiveRect").selectAll("g").remove();
    };
    if (dta.length < 100) { d3.select('#exclusionchartSteps').select('#gCompetitive').selectAll('g').style('opacity', '0.4'); }
    else { d3.select('#exclusionchartSteps').select('#gCompetitive').selectAll('g').style('opacity', '0.4'); }
  ...
  fillCompetivesRect(data: any) {
    ...
    d3.select("#excluCircle_main_steps #exclugCompetitiveRect").selectAll("g").remove();
    d3.select("#excluCircle_main_steps #exclugCompetitiveRect").raise();
    gs = d3.select("#excluCircle_main_steps #exclugCompetitiveRect");
