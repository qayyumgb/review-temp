...
export class PerformanceComponentsComponent implements OnInit, OnDestroy,AfterViewInit{
 ...
 /* performance*/
  ...
  RangeFilterGrid(IndexN:any, data:any) {
    var that = this;
    //if (IndexN == "1" && (that.ascending_val == true)) { data = data.sort(function (x:any, y:any) { return d3.ascending(escape(x.companyName.toUpperCase()), escape(y.companyName.toUpperCase())); }); }
    //else if (IndexN == "1" && (that.ascending_val == false)) { data = data.sort(function (x:any, y:any) { return d3.descending(escape(x.companyName.toUpperCase()), escape(y.companyName.toUpperCase())); }); }
    //else if (IndexN == "2" && (that.ascending_val == true)) { data = data.sort(function (x:any, y:any) { return d3.ascending(parseFloat(x.marketCap), parseFloat(y.marketCap)); }); }
    //else if (IndexN == "2" && (that.ascending_val == false)) { data = data.sort(function (x:any, y:any) { return d3.descending(parseFloat(x.marketCap), parseFloat(y.marketCap)); }); }
    //else if (IndexN == "3" && (that.ascending_val == true)) { data = data.sort(function (x:any, y:any) { return d3.ascending(x.ticker, y.ticker); }); }
    //else if (IndexN == "3" && (that.ascending_val == false)) { data = data.sort(function (x: any, y: any) { return d3.descending(x.ticker, y.ticker); }); }
    //else if (IndexN == "FV_asc" && (that.ascending_val == true)) { data = data.sort(function (x: any, y: any) { return d3.ascending(escape(x.factorValue), escape(y.factorValue)); }); }
    //else if (IndexN == "FV_asc" && (that.ascending_val == false)) { data = data.sort(function (x: any, y: any) { return d3.descending(escape(x.factorValue), escape(y.factorValue)); }); }
    //else if (IndexN == "erf_Score" && (that.ascending_val == true)) {
    //    data = [...data.sort(this.sortwithNullScore(that.ascending_val, 'scores'))];
    //    data = this.findMyHandleIndex_H(data);
    //    data = [...data.sort(this.sortwithNullScore(that.ascending_val, 'scores'))];
    //}
    //else if (IndexN == "erf_Score" && (that.ascending_val == false)) {
    //  data = [...data.sort(this.sortwithNullScore(that.ascending_val, 'scores'))];
    //  data = this.findMyHandleIndex_H(data);
    //  data = [...data.sort(this.sortwithNullScore(that.ascending_val, 'scores'))];
    //}
  ...
  ngOnInit() {
    ...
    var custSelectFactor = that.cusIndexService.custSelectFactor.subscribe(res => {
      ...
      this.onSort();
      //if (isNotNullOrUndefined(res) && isNotNullOrUndefined(res['id']) && res['id'] > 0) {
      //  this.SelFilter = 'FV_asc';
      //  this.ascending_val = false;
      //  if (res['id'] == 18) {
      //    this.ascending_val = true;
      //    this.SelFilter = 'erf_Score';
      //  }
      //  this.onSort();
      //} else {
      //  this.SelFilter = 4;
