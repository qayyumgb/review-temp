...
@Component({
  selector: 'approved-review-factsheet',
...
export class ReviewFactsheetComponent {
  ...
  onToggleVerify(event: any) {
    ...
    this.sharedData.userEventTrack('Approved Strategies', ("Account id:" + getAccId), ("Account id:" + getAccId), 'Enable For Trading Btn clicked');
  }
  ...
  right_spinner_off() {
    ...
    if (that.bldMyIndexMntHPortfolioData.length > 0 && that.factsheetPerfData.length > 0) {
      ...
      setTimeout(function () {
        ...
        try {
          if ($(".taxGainLoss").length > 0 && Math.round($(".taxGainLoss").scrollLeft())  + Math.round($(".taxGainLoss").innerWidth())  >= ($(".taxGainLoss")[0].scrollWidth) - 1) {
  ...
  downloadMethodology() {
    ...
    this.sharedData.userEventTrack('Approved Strategies', ("Account id:" + accID), ("Account id:" + accID), 'review factsheet Methodology download');
    this.showLoad1 = true;
  ...
  downloadPDF() {
    ...
    this.sharedData.userEventTrack('Approved Strategies', ("Account id:" + accID), ("Account id:" + accID), 'review factsheet download');
  ...
  download() {
    ...
    this.sharedData.userEventTrack('Approved Strategies', ("Account id:" + getAccId), ("Account id:" + getAccId), 'review factsheet download btn click');
  ...
  allowTade_fact() {
    ...
    this.sharedData.userEventTrack('Approved Strategies', tp, tp, 'Index Factsheet Trade now Click');
  ...
  downloadHoldingsExcel() {
    ...
    var basedata = this.dirIndexService.customizeSelectedIndex_Direct.value;
    this.sharedData.userEventTrack('Approved Strategies', 'Holdings History', (basedata['name'] + " Account id:" + accID), 'Export Holdings History Click');
  }
  ...
  downloadHisReturn() {
    ...
    this.sharedData.userEventTrack('Approved Strategies', 'Returns History', (basedata['name'] + " Account id:" + getAccId), 'Export Returns History Click');
  }
  ...
  saveproposal() {
    ...
    if (flg == false) {
      ...
      var basedata = this.dirIndexService.customizeSelectedIndex_Direct.value;
      var getAccId: any = parseInt(that.dirIndexService.selectedDirIndFactsheet.value);
      this.sharedData.userEventTrack('Approved Strategies', 'Proposal', (basedata['name'] + " Account id:" + getAccId), 'Export Proposal Click');
    }
  };
}
