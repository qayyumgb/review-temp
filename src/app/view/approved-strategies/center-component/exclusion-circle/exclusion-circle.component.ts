...
@Component({
  selector: 'approved-exclusion-circle',
...
export class ExclusionCircleComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    var directIndexData_sub = this.dirIndexService.directIndexData.subscribe((resData: any[]) => {
      this.circleData = resData;
      this.loadCircle(resData);
    });
    this.subscriptions.add(directIndexData_sub);
    var selCompany_sub = this.dirIndexService.returnAsOfDate.subscribe((date: any) => {
      try {
        this.returnAsof = date;
      } catch (e) { this.returnAsof = ''; }
    });
    var selCompany_sub = this.dirIndexService.selCompany.subscribe((resData: any) => {
      try {
        ...
        else { d3.select("#excluCircle_di").selectAll("#cSlider").remove(); }
        this.loadSel();
  ...
  loadCircle(data: any) {
    if (isNotNullOrUndefined(data) && data.length > 0) {
      ...
      this.circleData = [...da];
      this.fillCompetives([...da]);
      this.CreateComps('#excluCircle_di', [...da]);
      this.creatGradienArc(0, 100, [...data], "", '#excluCircle_di');
      var Bdata = this.dirIndexService.currentSList.value;
      if (isNotNullOrUndefined(Bdata)) {
        var selectSectorName = Bdata.srName;
  ...
  loadSel() {
    ...
    var stockKey: any = (isNotNullOrUndefined(this.dirIndexService.selCompany.value) && isNotNullOrUndefined(this.dirIndexService.selCompany.value['stockKey'])) ? this.dirIndexService.selCompany.value['stockKey'] : 0;
    ...
    if (filComp.length > 0) {
      ...
      var txt_m: string = (isNotNullOrUndefined(d.companyName)) ? d.companyName : (isNotNullOrUndefined(d.compname)) ? d.compname : "";
  ...
  fillCompetives(dta: any[]) {
    ...
    d3.select("#excluCircle_di #crlChart").select("#gCompetitive").remove();
    d3.select("#excluCircle_di #crlChart").select("#exclugCompetitiveRect").remove();
    gs = d3.select("#excluCircle_di").select('#crlChart').append("g").attr("id", "gCompetitive");
    d3.select("#excluCircle_di").select('#crlChart').append("g").attr("id", "exclugCompetitiveRect");
    ...
    ggs.on("pointerenter", function (event, d) { d3.select(this).classed("on", true); })
      ...
      .on("click", function (event, d) { that.dirIndexService.selCompany.next(d); });

    //if (data.length > fillMaxlen) { that.fillCompetivesRect(data, dta, mode); } else { d3.select("#excluCircle_di #exclugCompetitiveRect").selectAll("g").remove(); };
  ...
  creatGradienArc(sMin = 0, sMax = 100, datas: any, path = "", masterID: any) {
    var scale = d3.scaleLinear<any>().domain([0, datas.length]).range([0, 100]);
    var data: any = [...datas].map((item: any, i: any) => ({ score: (isNotNullOrUndefined(item['scores']) ? item['scores'] * 100 : scale(i)), ...item }));
    ...
    if (datas.length > 0) {
      ...
      if (!that.dirIndexService._notifyDiClick) {
  ...
  ngOnDestroy() {
    ...
    this.dirIndexService.directIndexData.next([]);
  ...
  creatClockSlider(selComp: any) {
    let that = this;
    d3.select("#excluCircle_di").selectAll("#cSlider").remove();
    if (isNotNullOrUndefined(selComp) && isNotNullOrUndefined(selComp.stockKey)) {
      var r: any = 150;
      r = parseInt(r);
      d3.select("#excluCircle_di").selectAll("#cSlider").remove();

      var g = d3.select("#excluCircle_di").select("#crlChart").append("g").attr("id", "cSlider").attr('transform', 'rotate(' + (-90) + ')')
  ...
  SetClockHandle(d: any) {
    ...
    let val = d.cx;
    var sliTag = d3.select("#excluCircle_di").select("#cSlider");
    var r: any = 150;
    ...
    sliTag.select("#cSliderCancel").on('click', function () { that.dirIndexService.selCompany.next(undefined); });
    ...
    d3.select("#excluCircle_di").select('.Sel_midCompS1').select('.SelCompName').attr('text-anchor', 'middle').attr('fill', 'var(--prim-button-color)').style('font-size', function () { if ((midtxt.length) > 30) { return "10px" } else { return "12px"; } })
    ...
    d3.select("#excluCircle_di").select('.Sel_midCompS1').select('.SelCompScore').attr('text-anchor', 'middle').style("fill", function () { return that.cl(medVel + 1); })
