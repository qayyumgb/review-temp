...
@Component({
  selector: 'approved-default-circle',
  templateUrl: './default-circle.component.html',
  styleUrl: './default-circle.component.scss'
})
export class DefaultCircleComponent implements OnInit {

  radius = 150;
  showSearchLoader: boolean = false;
  constructor(public sharedData: SharedDataService, public dirIndexService: DirectIndexService) { }
  ...
  ngOnInit(): void {
    ...
    var unSub = this.dirIndexService.directIndexBrcmData.subscribe((res: any) => {
      that.fillSectorName(res);
    });
    this.subscriptions.add(unSub);
  }
  fillSectorName(breadcrumbdata: any) {
    var that = this;
    var Bdata = breadcrumbdata[breadcrumbdata.length - 1];
    if (isNotNullOrUndefined(Bdata)) {
      var selectSectorName = breadcrumbdata[breadcrumbdata.length - 1].name;
      ...
      var selectSectorNameD = "Approved Strategies";
      d3.selectAll(".slider1 .mainsectorTxt .med__title").text(selectSectorNameD)
        .attr('y', function () { if ((selectSectorNameD.length) > 18) { return '125' } else { return '140' } })
  ...
  createCenterCicle(data: any) {
    ...
    try {
      if (data.length > 0) {
        that.creatGradienArc(0, 100, [...data], '#excluDirectCircle');
        that.CreateComps('#excluDirectCircle', [...data], "lvlTemp1");
  ...
  fillCompetives(dta: any[], animate = true) {
    ...
    d3.select("#DirectcrlChart").select("#gCompetitive").remove();
    d3.select("#DirectcrlChart").select("#TempchartCompetitiveRect").remove();
    gs = d3.select("#DirectcrlChart").append("g").attr("id", "gCompetitive");
    d3.select("#DirectcrlChart").append("g").attr("id", "TempchartCompetitiveRect");
    ...
    if (dta.length < 100) { d3.select('#exclusionDirectChart').select('#gCompetitive').selectAll('g').style('opacity', '0.6').style('cursor', 'pointer'); }
    else { d3.select('#exclusionDirectChart').select('#gCompetitive').selectAll('g').style('opacity', '0.4').style('cursor', 'pointer'); }
  ...
  creatGradienArc(sMin = 0, sMax = 100, datas: any, masterID: any) {
    ...
    gArc = d3.select(masterID).select("#DirectcrlChart").append("g").attr("id", "gradArc");
  ...
  CreateComps(masterID: any, dta: any, lvl: any) {
    let that = this;
    var compLst;
    var oSvg = d3.select(masterID).select("#DirectcrlChart");
  ...
  fillCompClick(d: any) { this.creatClockSlider([...this.selResData], d) }

  creatClockSlider(dta: any, selComp: any) {
    d3.selectAll("#cSlider").remove();
    var r: any = 150;
    d3.selectAll("#cSlider").remove();
    var g = d3.select("#DirectcrlChart").append("g").attr("id", "cSlider").attr('transform', 'rotate(' + (-90) + ')').on('mousedown', onDown).on("touchstart", onDown)
  ...
  SetClockHandle(data: any, d: any) {
    ...
    d3.select("#cSlider").on('pointerenter', function () {
      d3.select("#cSlider").select("#cSliderCancel").style('display', 'block');
    })
