...
declare var $: any;
@Component({
  selector: 'approved-performance-components',
...
export class PerformanceComponentsComponent implements OnInit, OnDestroy,AfterViewInit {
  ...
  ngOnInit() {
    ...
    var directIndexData_sub = this.dirIndexService.directIndexData.subscribe((resData: any[]) => {
      if (isNotNullOrUndefined(resData) && resData.length > 0) {
        ...
        this.sortProductsAsc(); // default to decending marketCap
    ...
    //    }
    //    // console.log('custom Pin', filterCustom)
  ...
  sortProductsAsc() {
    var that = this;
    that.directIndexGridSortedAsc = true;
    that.sortedDirectIndexGridData = [...that.directIndexGridData].sort(function (x: any, y: any) { return d3.descending(parseFloat(x.marketcap), parseFloat(y.marketcap)); });
...
//    @if (currentSList.pbGroupType != undefined && currentSList.pbGroupType == 'Index') {
//      <span class="basedOn" > {{ currentSList.basedOn }
//    } </span>
//  } @else{
//  <span class="basedOn" > {{ currentSList.basedOn }
//}& nbsp; ({{ currentSList.shortname }})</span>
    //    }
    if (isNotNullOrUndefined(selIndex['basedon']) && selIndex['pbGroupType'] == 'Index') { this.indexName = selIndex['basedon']; }
    else { this.indexName = selIndex['shortname']; }
