...
declare var $: any;
@Component({
  selector: 'approved-build-your-index',
...
export class BuildYourIndexComponent implements OnInit, OnDestroy {
  ...
  openIndexConstruction() {
    ...
    var clickeddata: any = [];
    this.dialog.open(IndexConstructionPopupComponent, { width: "50%", height: "auto", maxWidth: "100%", maxHeight: "90vh",
    panelClass:'indexcustommodalbox',
     data: { dialogTitle: title, dialogData: [], selectedIndex: this.dirIndexService._currentSList } });
    //}
  ...
  GetStrategyAccount(slid: any, post: boolean = false) {
    ...
    }, (error: any) => { this.diStrFactsheetAccData = []; });
          ...
          res.data.forEach((acc: any) => {
            if (acc['factsheetAvail'] == 'Y' || (acc['factsheetAvail'] == 'N' && acc['notifyId'] == 0)) {
              postData.push({
                ...
                "tenYrFlag": 1,
                "erfflag": that.dirIndexService.checkErfflag(clist)
              });
            }
          });
  ...
  PostRemoveAccLinks() {
    ...
    var PostRemoveAccLinks=this.dataService.PostRemoveAccLinks(statdata).pipe(first()).subscribe(data => {
      if (data[0] != "Failed") {
        that.sharedData.getNotificationDataReload();
        that.accountInit(that.changeFormValue);
      }
    }, error => { });
    this.subscriptions.add(PostRemoveAccLinks);
  }
  removeAccount() {
    var that = this;
    try {
  ...
  openToolCustom() {
    ...
    }, error => { console.log(error); this.sharedData.showMatLoader.next(false); });
  ...
  GetDBPershing(accID: any) {
    ...
    return new Promise((resolve, reject) => {
      var GetSubAccounts_DBP=this.dataService.GetSubAccounts_DBPershing(accID).pipe(first()).subscribe((res: any) => {
      ...
      this.subscriptions.add(GetSubAccounts_DBP);
    });
  }
  marketValue: any;
  ...
  updateAccModal() {
    ...
    this.sharedData.userEventTrack('Approved Strategies', accID, accID, 'Account data update');
  }
  ...
  DIcalculate() {
    ...
    this.sharedData.userEventTrack('Approved Strategies', sel['name'], sel['id'], 'calculate btn click');
  }
  ...
  PostStrategyData() {
    ...
    if (isNotNullOrUndefined(clist)) {
      ...
      Obj['erfflag'] = that.dirIndexService.checkErfflag(clist);
      Obj['description'] = '';
      Obj['indextotalcount'] = 0;
      Obj['removedcount'] = 0;
      var statdata = [];
      statdata.push(Obj);
      var PostStrategyData=that.dataService.PostStrategyData(statdata).pipe(first())
        .subscribe((data: any) => {
          if (data[0] != "Failed") {
  ...
  openFactsheet(d: number) {
    ...
    if (!this.checkfactDisable(d) || d == 0) {
      ...
      this.sharedData.userEventTrack('Approved Strategies', ("Account id:" + d), ("Account id:" + d), 'open factsheet click');
  ...
  viewRemovestrategy() {
    ...
    if (xx.length > 0) {
      ...
      var PostUpdateStrategyDisplayQueue=this.dataService.PostUpdateStrategyDisplayQueue(statdata).pipe(first()).subscribe(
        data => {
      ...
      this.subscriptions.add(PostUpdateStrategyDisplayQueue);
    }
  }
  ...
  accountEvTrack(e: any, acc: any) {
    ...
    this.sharedData.userEventTrack('Approved Strategies', aEv, aEv, 'Account select checkbox');
  }
