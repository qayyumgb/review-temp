...
export class DashboardCentercircleComponentSpinnerInner implements OnInit {
  ...
  constructor(public sharedData: SharedDataService, private dataService: DataService, private gloSrcService: GlobalSearchService,public dialog: MatDialog,
    private toastr: ToastrService, private dirIndexService: DirectIndexService, private preBuiltService: PrebuildIndexService) {
 ...
 findIndName(dta:any, code:any) {
  ...
  fillCompetives(dta: any[], animate = true) {
    ...
    if (dta.length < 100) { d3.select('#exclusionTempchartS').select('#gCompetitive').selectAll('g').style('opacity', '0.6').style('cursor', 'auto'); }
    else { d3.select('#exclusionTempchartS').select('#gCompetitive').selectAll('g').style('opacity', '0.4').style('cursor', 'auto'); }
  ...
  gfillCompMouseover(gs: any, d: any, dta: any, ev: any) {
    ...
    var ggs = gs.append("g").attr("class", "gfillCompMouseover")
      .style("cursor", "auto").style("font-size", "9px").style("font-family", 'var(--ff-regular)')
  ...
  gfillCompMouseoverRotate(d: any, dta: any) {
    ...
    gs.selectAll(".gfillCompMouseover").remove();
    var ggs = gs.append("g").attr("class", "gfillCompMouseover")
      .style("cursor", "auto").style("font-size", "10px").style("font-family", 'var(--ff-bold)')
      .attr("transform", function () {
        if (d.cx <= 90) { return "rotate(" + (d.cx + 1.0) + ")"; } else { return "rotate(" + (d.cx - 1.0) + ")"; }
      });
    //d3.select(ev).raise();
...
maxDelayTimeout = 10000; // Maximum delay (fastest speed)
  ...
  startStockDataCalls() {
    ...
    const updateInterval = () => {
      ...
      //console.log('that.currentDelay ', that.currentDelay, this.showSpinBtn);
      if (!this.showSpinBtn) {
      ...
      if (!this.showSpinBtn) {
        setTimeout(() => {
          if (that.currentDelay < that.maxDelay) {
            that.currentDelay += that.currentDelay + 1.5;
    ...
    setTimeout(() => {
      ...
      d3.select('#TempcrlChartS').transition()
        .ease(d3.easeCubicOut).select('.compLstlvlTemp1').style("opacity", 1);
      d3.select('#circleLogo').style("opacity", 0);
      d3.select('#circleSelectedName')
        .transition() 
        .duration(500) 
        .ease(d3.easeSinIn) 
        .style('opacity', 1); 

      d3.select('#fil_STW_Price')
        .transition() 
        .duration(500) 
        .ease(d3.easeSinIn) 
        .style('opacity', 1);
      that.sharedData.rouletCircleHomeRotate.next(false);
      clearInterval(that.intervalId);
      that.createSliderOnce(that.currentValSpinner);
