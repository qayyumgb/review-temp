...
declare var $: any;
@Component({
  selector: 'equityDefaultCenterCircle',
...
export class CenterCircleComponent implements OnInit, OnDestroy {
  ...
  constructor(public sharedData: SharedDataService, public equityService: EquityUniverseService, public dirIndexService: DirectIndexService,
    public dialog: MatDialog, private logger: LoggerService, public dataService: DataService, private toastr: ToastrService) {
    var that = this;
  ...
  ngOnInit() {
    ...
    var breadcrumbdata: any = this.equityService.breadcrumbdata.subscribe((res: any) => {
      this.breadcrumbdata = [...res];
      this.STW_PriceRefre = false;

      if (this.Slide_count == 3 || that.equityService._SRValue > 0) {
        this.resetSlider();
      }
      this.Slide_count = 1;
      this.controlLeft();
      this.checkSP_centerslider(this.breadcrumbdata);
      //console.log(this.breadcrumbdata, this.checkSP_centerslider(this.breadcrumbdata));
      //if (!this.equityService.checkFillCompCreation()) { that.equityService.selComp.next(undefined) }
    ...
    var SRValue: any = this.equityService.SRValue.subscribe((res: any) => {
      ...
      } else { d3.select('#resetAL').style('display', 'block'); }
    });
    this.subscriptions.add(SRValue);

    var linechart: any = this.equityService.triggerCumulativeLinechart.subscribe((res: any) => { if (res) { this.openLineChart(); this.equityService.triggerCumulativeLinechart.next(false); } });
    this.subscriptions.add(linechart);

    var rightGridData: any = this.equityService.rightGridData.subscribe((res: any) => {
      this.rightGridData = [...res];
      this.equityService.SRValue.next(0);
      this.loadCircleData([...res]);
      this.checkoverpricedVal([...res]);
      this.slider1Txts([...res]);
      that.creatgArcLine([...res]);
    });
    this.subscriptions.add(rightGridData);

    var selCompany: any = this.equityService.selComp.subscribe((selComp: any) => {
      this.showCompDetails = false;
      this.STW_PriceRefre = false;
      this.glbSearch.reset();
        if (isNotNullOrUndefined(selComp)) {
          if (this.equityService._SRValue > 0 && this.equityService._SRValue < 100 && this.sharedData.checkMenuPer(2027, 2232) == 'Y') {
            this.resetSlider();
            if (this.equityService.rightGridData.value > 19) {
              d3.selectAll(".sliderDisp").style("display", "block");
            }
          }
          this.equityService.rightGridData.pipe(first()).subscribe(rGData => {
            that.Slide_count = 1;
            that.controlLeft();
  ...
  showSP500Icon: boolean = false;
  checkSP_centerslider(bdata: any) {
    const lastItem = bdata?.[bdata.length - 1];
    if (this.isShowLosers) { this.openAvoidingLosers(); this.isShowLosers = false; }
    if (isNotNullOrUndefined(lastItem) && lastItem.group === 'Index' && lastItem.assetId === 55555555 && this.sharedData.checkMenuPer(2027, 3284) == 'Y') { this.showSP500Icon =  true; }
    else { this.showSP500Icon =  false; }
  }
  isShowLosers: boolean = false;
  openAvoidingLosers() {
    this.resetSlider();
    this.equityService.selComp.next(undefined);
    this.isShowLosers = !this.isShowLosers;
  }
  checkErrorAction(errorList: any) {
    var checkError = errorList;
    if (checkError.destination == 'resetDrag') {
      this.resetSlider();
    }
  }
  slider1Txts(data:any) {
    ...
    if (isNotNullOrUndefined(data) && data.length > 0) {
      ...
      var medTxt: any = d3.median(data, function (d: any) { return d.score; });
      var med: any = d3.format(".1f")(medTxt);
      var color = this.equityService.checkMedColor(data, med);
      var Bdata = that.breadcrumbdata[that.breadcrumbdata.length - 1];
      if (isNotNullOrUndefined(Bdata)) {
        var selectSectorName = that.breadcrumbdata[that.breadcrumbdata.length - 1].name;
        d3.selectAll(".slider1 .mainsectorTxt .med__title").text(selectSectorName)
          .attr('y', function () { if ((selectSectorName.length) > 15) { return '125' } else { return '140' } })
          .style('font-size', function () { if ((selectSectorName.length) > 20) { if ((selectSectorName.length) > 25) { return '16px' } else { return '18px' } } else { if ((selectSectorName.length) >= 10) { return '22px' } else { return '30px' } } })
          .call(that.sharedData.Crlwrapping, 180, "top");
      } else {
        var selectSectorNameD = "Equity";
      ...
      this.circleCount = 0;
      d3.selectAll("#defCenCircle .med__value").text(0).style("fill", function () { return that.cl(0); });
    };
  }
  ...
  checkBoxFill(range: any) {
    if (isNotNullOrUndefined(range) && isNotNullOrUndefined(this.equityService.showHFCompanyRanges[range])) {
      return this.equityService.showHFCompanyRanges[range];
    } else {
      return false;
    }
  };
  creatClockSlider(dta:any, selComp:any) {
    let that = this;
    d3.selectAll("#cSlider").remove();
    if (isNotNullOrUndefined(selComp['impRev'])) { 
      // this.impRevValue = d3.format(".2f")(selComp['impRev']);
      this.impRevValue = (selComp['impRev'] * 100).toFixed(1);
    ...
    if (isNotNullOrUndefined(selComp.price)) {
      this.STW_Price = this.priceTofix(parseFloat(selComp.price));
      this.checkCurType(selComp, selComp.price.toFixed(1));
    }
    ...
    if (isNotNullOrUndefined(selComp) && isNotNullOrUndefined(selComp.isin)) {
      ...
      g.append("text").attr("class", "sText1").attr("x", + r + (40)).attr("y", 9).style("display", "none")
        .style("font-size", "8px").attr("fill", "var(--prim-button-color)").style("font-family", 'var(--ff-medium)').text("0.00");
      ...
      cancelBtn.select('g').append("path").attr('fill', 'var(--leftSideText-B)').attr("class", "bg_stroke")
  ...
  SetClockHandle(data: any, d: any, type: boolean = false) {
    ...
    d3.select("#cSlider").select("#cSliderCancel").on('click', function () {
      that.equityService.selComp.next(undefined);
      try {
        that.sharedData.userEventTrack('Equity Universe', that.equityService._selGICS.name, that.equityService._SRValue, 'company remove icon click');
      } catch (e) { }
    ...
    if (Cname.length > 15) { midtxt = Cname.slice(0, 15) + "... (" + d.ticker + ") "; }
    ...
    var medVel: any = d3.format(".1f")(d.score);
      var colorVal = that.checkHandleColor(that.equityService.rightGridData.value, d);
    ...
    //d3.select(".Sel_defComp").style('display', 'none');
    //d3.select(".Sel_midComp").style('display', 'block');
  ...
  circleHFclick(key: string) {
    var that = this;
    if (key == 'selectedPin') {
      if (that.equityService.showHFCompanyRanges.range0_25 == true || that.equityService.showHFCompanyRanges.range25_50 == true ||
        that.equityService.showHFCompanyRanges.range50_75 == true || that.equityService.showHFCompanyRanges.range75_100 == true) {
        this.equityService.showHFCompanyRanges[key] = !this.equityService.showHFCompanyRanges[key];
        this.HFCompanyArc();
      }
      else {
      ...
      this.equityService.showHFCompanyRanges[key] = !this.equityService.showHFCompanyRanges[key];
      this.HFCompanyArc();
      if (that.equityService.showHFCompanyRanges.range0_25 == false && that.equityService.showHFCompanyRanges.range25_50 == false &&
        that.equityService.showHFCompanyRanges.range50_75 == false && that.equityService.showHFCompanyRanges.range75_100 == false) {
        this.equityService.showHFCompanyRanges['selectedPin'] = false;
      }
    }
  }
  ...
  HFCompanyArc() {
    ...
    if (that.equityService.showHFCompanyRanges.selectedPin || this.Slide_count == 1) {
      if (that.equityService.showHFCompanyRanges.range0_25 == true) { d3.selectAll("#defCenCircle .HFCompRange0_25").classed("HF_active", true); }
      if (that.equityService.showHFCompanyRanges.range25_50 == true) { d3.selectAll("#defCenCircle .HFCompRange25_50").classed("HF_active", true); }
      if (that.equityService.showHFCompanyRanges.range50_75 == true) { d3.selectAll("#defCenCircle .HFCompRange50_75").classed("HF_active", true); }
      if (that.equityService.showHFCompanyRanges.range75_100 == true) { d3.selectAll("#defCenCircle .HFCompRange75_100").classed("HF_active", true); }
    }
  }
  ...
  loadCircleData(data:any) {
    ...
    //if (this.equityService.checkFillCompCreation()) { this.fillCompetives([...data]); }
    //else {
    //  if (!this.onkeyPress) {
    //    d3.selectAll('#cSlider').remove();
    //  }
    ...
    //};
    if (data.length > 19 && this.sharedData.checkMenuPer(2027, 2232) == 'Y') {
    ...
    d3.select("#defCenCircle #decile1").text(data.filter((x: any) => x.score >= 0 && x.score <= 25).length);
    d3.select("#defCenCircle #decile2").text(data.filter((x: any) => x.score >= 25 && x.score <= 50).length);
    d3.select("#defCenCircle #decile3").text(data.filter((x: any) => x.score >= 50 && x.score <= 75).length);
    d3.select("#defCenCircle #decile4").text(data.filter((x: any) => x.score >= 75 && x.score <= 100).length);
  }
  HomeRootPath = '';
  ...
  creatgArcLine(value:any) {
    var that = this;
    var UOpricestockPin = that.equityService.showHFCompanyRanges.UOpricestockPin;
    if (value.length > 0 && (this.Slide_count == 2 || UOpricestockPin)) {
  ...
  CreateComps(masterID:string, dta:any, lvl:string) {
    ...
    if (dta != null && dta.length > 0 && (that.equityService.SRValue.value == 0 || that.equityService.SRValue.value == 100)) {
      var len = dta.length;
    ...
    if (that.equityService._SRValue > 0 && that.equityService._SRValue < 100 && isNotNullOrUndefined(that.equityService._avoidLoserData['addedCompanies'])) {
      addedCompanies = [...that.equityService._avoidLoserData['addedCompanies']];
    }
    ...
    var compg = compC.selectAll("g")
      ...
      .on("pointerenter", function (ev: any, d: any) {
        ...
        if (that.equityService._SRValue > 0 && that.equityService._SRValue < 100 && isNotNullOrUndefined(d) && isNotNullOrUndefined(d['stockKey'])) {
    ...
    if (SelRes.length == 0) { SelRes = [...that.equityService._rightGridData]; }
    ...
    compg.append("rect").attr("height", "1px")
      .attr("class", function (d: any) {
        if (dta.length > 600) { return ("crect rectOpa " + that.equityService.HFCompanyTxt(d, that.equityService.showHFCompanyRanges)); }
        else { return ("crect " + that.equityService.HFCompanyTxt(d, that.equityService.showHFCompanyRanges)); }
      })
      .attr("fill", "#4b4b4b")
    ...
    that.HFCompanyArc();
    that.creatgArcLine(that.equityService._rightGridData);
  ...
  showCompOver(dat:any) {
    ...
    ggs.append("text")
      .attr("x", function (d) { return that.equityService.txtx1(dat); })
      .style("font-family", 'var(--ff-medium)')
      .style("fill", function (d) { return "#4b4b4b"; })
      .attr("transform", function (d) { return that.equityService.txttrans(dat); })
      .style("text-anchor", function (d) { return that.equityService.txtanch(dat); })
      .attr("class", function (d) { return ""; })
      .text(function (d) { return d3.format(",.1f")(dat.scores*100) + "%" });

    ggs.append("text")
      .attr("x", function (d) { return that.equityService.txtx(dat); })
      .style("fill", function (d) { return "#4b4b4b"; })
      .attr("transform", function (d) { return that.equityService.txttrans(dat); })
      .style("text-anchor", function (d) { return that.equityService.txtanch(dat); })
      .attr("class", function (d) { return ""; })
  ...
  fillCompetives(dta: any) {
    ...
    var grpG = ggs;
    grpG.append("text")
      .attr("x", function (d) { return that.equityService.txtx1(d); })
      .attr("id", "gcStxt")
      ...
      .style("text-anchor", function (d: any) { return that.equityService.txtanch(d); })
      .attr("class", function (d: any) {
    ...
    grpG.append("text")
      .attr("x", function (d) { return that.equityService.txtx(d); })
      .style("fill", function (d: any): any {
        if (dta.filter((x:any) => x.isin == d.isin).length == 0) { return "#1d397b"; }
      })
      .attr("transform", function (d: any): any { if ((d.cx) > 90) { return "rotate(180)"; } })
      .style("text-anchor", function (d) { return that.equityService.txtanch(d); })
      .attr("class", function (d: any) {
        if ((that.equityService.SRValue.value > 0 && that.equityService.SRValue.value < 100) && isNotNullOrUndefined(that.equityService._avoidLoserData['addedCompanies'])
    ...
    d3.select("#cSlider").raise();
    //if (that.showHFCompanyRanges[that.selHFCompanyPath].selectedPin == false && that.Slide_count != 1) { ggs.selectAll("text").classed("HF_active", false); }
  ...
  fillmouseOver(d:any) {
    if (+this.equityService.SRValue.value > 0 && +this.equityService.SRValue.value < 100) {
      if (this.equityService._avoidLoserData['addedCompanies'].filter((x: any) => x.stockKey == d.stockKey).length > 0) { this.showCompMOver(d); }
  ...
  showCompMOver(dat:any) {
    let that = this;
    var checkSelIsin: boolean = isNullOrUndefined(this.equityService._selComp);
    ...
    if (isNotNullOrUndefined(dat) && checkSelIsin) {
      ...
      var colorVal: any = that.checkHandleColor(that.equityService.rightGridData.value, dat);
      d3.select('.Sel_midCompS1').select('.SelCompScore').attr('text-anchor', 'middle').style("fill", '#d8dce5')
        .style('font-family', "var(--ff-semibold)").text(medVel + "%");
  ...
  fillCompetivesRect(data: any) {
    ...
    ggs.on("pointerenter", function (ev: any, d: any) {
      ...
      if (dst.length > 0) {
        ...
        } else { filD = that.equityService._avoidLoserData['addedCompanies'].filter((x: any) => x.stockKey == dst[0].stockKey); }
  ...
  gfillCompMouseover(gs: any, d: any, dta: any, ev: any) {
    ...
    d3.select(ev).raise();
    ggs.append("text").attr("x", that.equityService.txtx1(d)).attr("id", "gcStxt")
      ...
      .attr("transform", function (): any { if ((d.cx) > 90) { return "rotate(180)"; } }).style("text-anchor", that.equityService.txtanch(d))
      .attr("class", function () {
        if ((that.equityService.SRValue.value > 0 && that.equityService.SRValue.value < 100) && isNotNullOrUndefined(that.equityService._avoidLoserData['addedCompanies'])
          && that.equityService._avoidLoserData['addedCompanies'].filter((x: any) => x.isin == d.isin).length == 0) {
          if (that.equityService.showHFCompanyRanges['selectedPin'] == false && that.Slide_count != 1) {
          ...
          if (that.equityService.showHFCompanyRanges['selectedPin'] == false && that.Slide_count != 1) {
            ...
            return (that.equityService.HFCompanyTxt(d, that.equityService.showHFCompanyRanges) + " avbold");
          }
        };
    ...
    ggs.on("click", function () {
      if (that.equityService._SRValue == 0 || that.equityService._SRValue == 100) { that.equityService.selComp.next(d); }
      else {
        var fil: any = that.equityService.rightGridData.value.filter((x: any) => x.stockKey == d.stockKey);
        if (fil.length > 0) { that.equityService.selComp.next(fil[0]); }
      }
    ...
    //if (that.showHFCompanyRanges.selectedPin == false && that.Slide_count != 1) { ggs.selectAll("text").classed("HF_active", false); }
  ...
  circleRange(values:any) {
    ...
    function writeTimeInfo(sliderObject: any) {
      ...
      if ((sliderObject.a == 0 && sliderObject.e == 100) || (sliderObject.e == 0 && sliderObject.a == 0) || (sliderObject.e == 0 && sliderObject.a == 100)) {
        ...
        if (that.dragstart && that.equityService.SRValue.value >0) {
          that.dragstart = false;
          that.resetSlider();

        }
      } else {
        if (sliderObject.e < 100 && sliderObject.e > 0) { that.equityService.SRValue.next(parseInt(sliderObject.e.toFixed(0)))  }
        else if (sliderObject.a < 100 && sliderObject.a > 0) { that.equityService.SRValue.next(parseInt(sliderObject.a.toFixed(0)))  }
        d3.select('#gradArc').select('.circle_overlay').attr('stroke-dasharray', (that.equityService.SRValue.value) + " " + '100').style('display', 'block');
        that.SliderOnChange();
    ...
    if (that.equityService.SRValue.value < 100 && that.equityService.SRValue.value > 0) {
      var aScale = d3.scaleLinear().range([0, 360]).domain([100, 0]);
      var getAngel = aScale(that.equityService.SRValue.value);
      helper.graphdata = [{ angle: 360, label: "a", value: 0 }, { angle: getAngel, label: "e", value: that.equityService.SRValue.value }];
      handles.selectAll('.handlercontainer').data(helper.getData());
      updateHandles({ angle: getAngel, label: "e", value: that.equityService.SRValue.value });
      checkHandlesPosition();
    }
    d3.selectAll(".sliderDisp").style("display", "block");
    function drawHandles() {
      ...
      circles.append("text")
        ...
        .text(function (d: any) { if (that.equityService.SRValue.value < 96 && that.equityService.SRValue.value > 4 && d.label == 'e') return that.SRValue.toFixed(0); else return ''; });
    ...
    function drawTickers() {
      ...
      roTicker.select(".tick0").select("text").attr("transform", function (d) { return checkPoi == 1 ? "rotate(84)translate(0,8)" : "rotate(84)translate(0,8)" }).style("font-family", "var(--ff-medium)").style("fill", tickColor1).style("font-size", checkPoi == 1 ? "7px" : "8px");
      ...
      roTicker.select(lastticks).select("text").attr("transform", function (d) { return checkPoi == 1 ? "rotate(84)translate(-21,13)" : "rotate(84)translate(-8,8)" }).style("font-family", "var(--ff-medium)").style("fill", tickColor1).style("font-size", checkPoi == 1 ? "6px" : "7px");
      ...
      handles.style("display", "none").append('g').attr('transform', ('translate(0,' + (radius) * -1 + ')')).append('circle').attr('r', handleRadius).style('stroke', '#d0d4dc')
        ...
        .on('click', function (d) {
          ...
          try {
            that.sharedData.userEventTrack('Equity Universe', that.equityService._selGICS.name, that.equityService._SRValue, 'Avoid loser Reset btn Click');
          } catch (e) { }
        });
    }
  ...
  resetSlider() {
    let that = this;
    that.equityService.SRValue.next(0);
    that.equityService.rightGridData.next([...that.equityService.rightGridData.value]);
    if (that.equityService.rightGridData.value.length > 19) {
  ...
  checkTickerLine() {
    ...
    if ((this.uname.indexOf('technogradient.com') > -1) || (this.uname.indexOf('newagealpha.com') > -1)) {
      let SelIndId = that.checkSelIndId(that.breadcrumbdata);
      var GICSId = 0;
      var CType = 'MC';
      var IndexId = this.equityService.getMyIndexId(this.equityService._selGICS);
      if (SelIndId == 0) { GICSId = 0 }
      else { GICSId = that.equityService._breadcrumbdata[that.equityService._breadcrumbdata.length - 1].code; }
  ...
  SliderOnChange() {
    ...
    this.equityService.GetAlloc().then((AllocData: any) => {
      this.equityService.currentAllocation(AllocData).then((res: any) => {
        var AllocDta: any = [...res['resData']];
        var addedCompanies:any = [...res['addedCompanies']];
        var removedCompanies:any = [...res['removedCompanies']];
        that.equityService.avoidLoserData.next({ 'addedCompanies': addedCompanies, 'removedCompanies': removedCompanies, resData: AllocDta });
       ...
       // if (that.equityService.checkFillCompCreation()) {
        ...
        this.equityService.checkAllocAlertlist().then((res) => {
          if (isNotNullOrUndefined(res) && res) {
            if (that.equityService._SRValue > 0 && that.equityService._SRValue < 100 && this.sharedData.checkMenuPer(2027, 2233) == 'Y') { this.equityService.UpdateAllocDialog() }
          }
        });
        //that.sharedData.checkHoldingDate();        
      });
      that.equityService.getAnnumValue().then((res: any) => {
        //console.log(that.equityService._SRValue);
        if (isNotNullOrUndefined(res) && res.length > 0 && that.equityService._SRValue > 0) {
          that.equityService.getCumulativeAnnData.next(res);
      ...
      try {
        this.sharedData.userEventTrack('Equity Universe', this.equityService._selGICS.name, this.equityService._SRValue, 'Avoid loser draged');
      } catch (e) { }
    });
    if (this.showSP500Icon && this.isShowLosers) {
      var dragRange = "top" + String(100 - that.equityService._SRValue);
      var PostIndexPreRunslanding = this.dataService.PostIndexPreRunslanding(dragRange).pipe(first()).subscribe((res: any) => {
        if (isNotNullOrUndefined(res) && res.length > 0) {
          this.draggedDataSP500 = res;
          this.worthReturnSP = this.draggedDataSP500[0].range.toFixed(2);
          this.addReturnSP = (((this.draggedDataSP500[0].range / 1000 - 1) - (this.draggedDataSP500[0].top100 / 1000 - 1)) * 100).toFixed(0);
        } else { this.draggedDataSP500 = [] }
      }, (error: any) => { this.logger.logError(error, 'getIndexPreRuns'); this.draggedDataSP500 = [] });
    }
  }
  draggedDataSP500: any = [];
  worthReturnSP: any;
  addReturnSP: any;
  openLineChart() {
    ...
    try {
      ...
      var options = {
        ...
        SRValue: this.equityService.SRValue.value,
      ...
      var dilog= this.dialog.open(LineChartComponent, {
        ...
        if (isNotNullOrUndefined(response['SRValue'])) {
          ...
          if (this.equityService._SRValue != SRValue) {
            if (SRValue > 0 && SRValue < 100) {
              this.equityService.SRValue.next(SRValue);
    ...
    try {
      this.sharedData.userEventTrack('Equity Universe', this.equityService._selGICS.name, this.equityService._SRValue, 'Avoid loser linechart popup click');
    } catch (e) { }
  }
  getAnalysisSlider() {
    var that = this;
  ...
  checkCurType(Currency: any, value: any) {
    this.STW_Price = this.priceTofix(value);
    this.STW_Pricecurrency = Currency.currency + ' ' + d3.format(",.1f")(value);
  }
  onkeyPress: boolean = false;
  onKeyEnterPrice(event: any) {
    let that = this;
    if (isNotNullOrUndefined(event.target.value)) {
      var value = parseFloat(event.target.value);
      if (isNullOrUndefined(value) || value == 0 || isNaN(value)) {
        this.STW_PriceRefre = false;
        this.STW_PriceRef('');
      } else {
        var GNPC=this.dataService.GetNAAPriceChange(this.equityService._selComp['stockKey'], value).pipe(first())
          .subscribe((res: any) => {
            this.STW_PriceRefre = true;
            let localdata:any = [];
            [...that.equityService._rightGridData].forEach(val => localdata.push(Object.assign({}, val)));
            let selcomp = localdata.filter((x: any) => x.isin === that.equityService._selComp['isin']);
            selcomp[0].scores = parseFloat(res[0]);
            selcomp[0].score = selcomp[0].scores * 100;
            selcomp[0].deg = selcomp[0].score;
            localdata = localdata.sort(function (x: any, y: any) { return d3.ascending(parseFloat(x.scores), parseFloat(y.scores)); });
            localdata.forEach(function (d: any, i: any) {
              d.cx = ((i * 360 / localdata.length) - 90);
              return d
            });
            that.onkeyPress = true;
            that.loadCircleData([...localdata]);
            that.SetClockHandle(localdata, selcomp[0], true);
          }, (error: any) => {
            that.logger.logError(error, 'GetNAAPriceChange');
            this.STW_PriceRefre = false;
            this.STW_PriceRef('');
            that.onkeyPress = false;
          });
        this.subscriptions.add(GNPC);
      }
    } else {
      this.STW_PriceRefre = false;
      this.STW_PriceRef('');
  ...
  STW_PriceRefre: boolean = false;
  STW_PriceRef(event:any) {
    var that = this;
    this.STW_PriceRefre = !this.STW_PriceRefre;
    if (this.STW_PriceRefre) { } else {
      this.checkCurType(this.equityService._selComp, this.equityService._selComp.price.toFixed(1));
      [...that.equityService._rightGridData].forEach(function (d, i) {
        d.cx = ((i * 360 / [...that.equityService._rightGridData].length) - 90);
        return d
      });
      var selInd = [...that.equityService._rightGridData].filter(x => x.isin == that.SelISIN);
      if (isNotNullOrUndefined(selInd) && selInd.length > 0) {
        //that.stockPriceSelInd = selInd;
        //var score = selInd[0].score.toFixed(1);
        //d3.select("#ToolSvgSlider").selectAll('.homeScore2').style('display', 'flex').text(score).style("fill", function () { return that.cl(score); });
        that.SetClockHandle([...that.equityService._rightGridData], selInd[0])
      }
      that.CreateComps('#defCenCircle', [...that.equityService._rightGridData], "lvl1");
    }
  }

  priceTofix(price: any) { return parseFloat(d3.format(".1f")(price)) };
  ...
  openImpliedRevenue() {
    var that =this;
    var ind: any = this.equityService._breadcrumbdata.filter((x: any) => x['group'] == "Index");
    var filterTicker = this.equityService._selComp['ticker'] == this.sharedData._showNavPPT_Ticker;
    var currentPPTSlide = this.sharedData._GetDefaultPresentationdata;
    if(!filterTicker){
      if(that.sharedData._GetAllPresentationtickers.length>0){
         var pptfilterTicker =  that.sharedData._GetAllPresentationtickers.filter((x:any)=> x.ticker == this.equityService._selComp['ticker']);
        if (pptfilterTicker.length > 0 && currentPPTSlide.length > 0) {
          if (currentPPTSlide[0].presentationName == pptfilterTicker[0].presentationName) { filterTicker = true; }
          else { filterTicker = false; }
        } else { filterTicker = false; }
      }
    }
    var IGRV_permission = this.sharedData.checkShowLeftTab(2032) == 'A' && this.sharedData.checkMenuPer(2032, 2264) == 'Y' &&
      this.sharedData.checkMenuPer(2032, 2267) == 'Y' && filterTicker && this.sharedData._getPresentationMode == 'Y';
    ...
    try {
      ...
      var options = {
        ...
        selGICS: this.equityService.selGICS.value,
        CType: 'MC',
        showPPTComparison: IGRV_permission
      }
      that.dialog.open(ImpliedRevenueComponent, {
  ...
  private _filter(value: any) {
    ...
    try {
      if (that.equityService._SRValue > 0 && that.equityService._SRValue < 100 && isNotNullOrUndefined(that.equityService._avoidLoserData['addedCompanies'])) {
        data = [...that.equityService._avoidLoserData['addedCompanies']];
      }
    } catch (e) { }
  ...
  circleSrcEnter(ev: any) {
    if (isNotNullOrUndefined(ev) && isNotNullOrUndefined(ev.option) && isNotNullOrUndefined(ev.option.value)) {
      ...
      if (isNotNullOrUndefined(value) && value instanceof Object) { this.equityService.selComp.next(value); };
      this.glbSearch.reset();
    }
  }
