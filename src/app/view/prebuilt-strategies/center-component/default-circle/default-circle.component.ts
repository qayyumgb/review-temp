...
@Component({
  selector: 'pre-default-circle',
  templateUrl: './default-circle.component.html',
  styleUrl: './default-circle.component.scss'
})
export class DefaultCircleComponent_Prebuilt implements OnInit {

  radius = 150;
  showSearchLoader: boolean = false;
  constructor(public sharedData: SharedDataService, public preIndexService: PrebuildIndexService) { }
  ...
  fillSectorName(breadcrumbdata: any) {
    ...
    if (isNotNullOrUndefined(Bdata)) {
      ...
      var selectSectorNameD = "Prebuilt";
  ...
  createCenterCicle(data: any) {
    ...
    try {
      if (data.length > 0) {
        that.creatGradienArc(0, 100, [...data], '#excluPrebuildCircle');
        that.CreateComps('#excluPrebuildCircle', [...data], "lvlTemp1");
  ...
  fillCompetives(dta: any[], animate = true) {
    ...
    d3.select("#PrebuildcrlChart").select("#gCompetitive").remove();
    d3.select("#PrebuildcrlChart").select("#TempchartCompetitiveRect").remove();
    gs = d3.select("#PrebuildcrlChart").append("g").attr("id", "gCompetitive");
    d3.select("#PrebuildcrlChart").append("g").attr("id", "TempchartCompetitiveRect");
    ...
    if (dta.length < 100) { d3.select('#exclusionPrebuildChart').select('#gCompetitive').selectAll('g').style('opacity', '0.6').style('cursor', 'pointer'); }
    else { d3.select('#exclusionPrebuildChart').select('#gCompetitive').selectAll('g').style('opacity', '0.4').style('cursor', 'pointer'); }
  ...
  creatGradienArc(sMin = 0, sMax = 100, datas: any, masterID: any) {
    ...
    gArc = d3.select(masterID).select("#PrebuildcrlChart").append("g").attr("id", "gradArc");
  ...
  CreateComps(masterID: any, dta: any, lvl: any) {
    let that = this;
    var compLst;
    var oSvg = d3.select(masterID).select("#PrebuildcrlChart");
  ...
  creatClockSlider(dta: any, selComp: any) {
    ...
    var r: any = 150;
    d3.selectAll("#cSlider").remove();
    var g = d3.select("#PrebuildcrlChart").append("g").attr("id", "cSlider").attr('transform', 'rotate(' + (-90) + ')').on('mousedown', onDown).on("touchstart", onDown)
