...
@Component({
  selector: 'pre-exclusion-circle',
  templateUrl: './exclusion-circle.component.html',
  styleUrl: './exclusion-circle.component.scss'
})
export class ExclusionCircleComponent_Prebuilt implements OnInit, OnDestroy {
  radius = 150;
  circleData: any = [];
  ...
  constructor(public sharedData: SharedDataService, public preBuiltService: PrebuildIndexService, public dialog: MatDialog,) {
    var that = this;
  ...
  impRevValue: any = '';
  checkSelComp(d:any) {
    this.showCompInfo = false;
    this.glbSelComp = undefined;
    if (isNotNullOrUndefined(d)) {
      var filComp = [...this.sharedData.selResData.value].filter((x: any) => x.ticker == d['ticker']);
      if (filComp.length > 0) {
        this.showCompInfo = true;
        this.glbSelComp = filComp[0];
        this.selcore = (d3.format(".1f")(this.glbSelComp.scores * 100) + "%");
        this.STW_Price = (isNotNullOrUndefined(this.glbSelComp['currency']) ? this.glbSelComp['currency'] : '') + ' ' + d3.format(",.1f")(this.glbSelComp.price);
        this.impRevValue = (this.glbSelComp['impRev'] * 100).toFixed(1);
      }
    }
  }
  ...
  openImpliedRevenue() {
    var that = this;
    var ind: any = this.preBuiltService._prebuiltIndexBrcmData.filter((x: any) => x['group'] == "Index");
    var filterTicker = this.glbSelComp['ticker'] == this.sharedData._showNavPPT_Ticker;
    if (!filterTicker) {
      if (that.sharedData._GetAllPresentationtickers.length > 0) {
        var pptfilterTicker = that.sharedData._GetAllPresentationtickers.filter((x: any) => x.ticker == this.glbSelComp['ticker']);
        if (pptfilterTicker.length > 0) {
          filterTicker = true;
        }
      }
    }
    ...
    var IGRV_permission = this.sharedData.checkShowLeftTab(2032) == 'A' && this.sharedData.checkMenuPer(2032, 2264) == 'Y' &&
      this.sharedData.checkMenuPer(2032, 2267) == 'Y' && filterTicker;
    ...
    try {
      ...
      var options = {
        ...
        selComp: this.glbSelComp,
        breadcrumb: this.preBuiltService.prebuiltIndexBrcmData.value,
        //selGICS: this.preBuiltService.selGICS.value,
        CType: 'MC',
  ...
  showCompInfo: boolean = false;
  private _filter(value: any) {
    let that = this;
    if (isNullOrUndefined(value) || value instanceof Object) { value = '' };
    if (isNotNullOrUndefined(value)) { value = value.toLowerCase(); };
    var data: any = [...this.circleData];
    var compFilter: any = [...data].filter((res: any) => (res['compname'].toLowerCase().includes(value) || res['ticker'].toLowerCase().includes(value)));
  ...
  circleSrcEnter(ev: any) {
    if (isNotNullOrUndefined(ev) && isNotNullOrUndefined(ev.option) && isNotNullOrUndefined(ev.option.value)) {
      ...
      if (isNotNullOrUndefined(value) && value instanceof Object) { this.preBuiltService.selCompany.next(value) };
  ...
  onloadIndexData(data: any) {
    ...
    try {
      ...
      var medTxt: any = d3.median([...data], function (d: any) { return d.scores; });
      var med: any = d3.format(".1f")(medTxt * 100);
      var color = (medTxt * 100);
      d3.selectAll(".slider1 .med__value").text(med).style("fill", function () { return that.cl(color); });
      setTimeout(() => { that.loadTitle(); },100);
    } catch (e) { console.log(e) }
  }
  ...
  loadTitle() {
    var that = this;
    try {
      if (this.showCompInfo) {
        var dt = this.preBuiltService.selCompany.value;
        if (isNotNullOrUndefined(dt) && isNotNullOrUndefined(dt.compname)) {
          let txt1 = " (" + dt.ticker + ") ";
          let txt :any= dt.compname;
          if ((txt.length) > 18) { txt = txt.slice(0, 18).trim() + "..."; }
          txt = txt + txt1;
          d3.selectAll(".slider1 .mainsectorTxt .med__title").text(txt).attr('y', '0')
            .style('font-size', '10px').call(that.sharedData.Crlwrapping, 180, "top");
        }
      } else {
        var Bdata = this.preBuiltService.customizeSelectedIndex_prebuilt.value;
        if (isNotNullOrUndefined(Bdata)) {
          var selectSectorName = Bdata.indexname;
          //if ((selectSectorName.length) > 18) { selectSectorName = selectSectorName.slice(0, 18).trim() + "..."; }
          if (Bdata.CommonTicker != undefined && Bdata.CommonTicker != '') {
            var tick = '('+Bdata.CommonTicker+')';
            selectSectorName = selectSectorName + tick;
          }
          d3.selectAll(".slider1 .mainsectorTxt .med__title").text(selectSectorName)
            .attr('y', function () { if ((selectSectorName.length) > 20) { return '-10' } else { return '0' } })
  ...
  fillCompetives(dta: any[]) {
    ...
    var ggs = gs.selectAll("g")
      .data(sdata)
      .enter().append("g")
      .attr("id", function (d) { return 'Cu_' + d.stockkey; })
    ...
    ggs.on("pointerenter", function (event, d) { d3.select(this).classed("on", true); })
      ...
      .on("click", function (event, d) { that.preBuiltService.selCompany.next(d); });
    ...
    d3.select("#excluCircle_pre").selectAll("#cSlider").raise();
  }
  ...
  creatGradienArc(sMin = 0, sMax = 100, datas: any, path = "", masterID: any) {
    let that = this;
    var data: any = [...datas].map((item: any) => ({ score: (item.scores * 100), ...item })).sort(function (x: any, y: any) { return d3.ascending(x.scores, y.scores); });
    ...
    var gArcLine = null;
    var arcType = "score";
    //gC100 = d3.scaleLinear<any>().domain([0, 100]).range(this.sharedData.cirArcGrdClr);
  ...
  SetClockHandle(d: any) {
    let that = this;
    let txt = d.compname;
    let txt1 = "(" + d.ticker + ") " + d3.format(".1f")(d.scores*100) + "%";
    //if (that.showCircleScores) {
    //  txt1 = "(" + d.ticker + ") " + d3.format(".1f")(d.scores) + "%";
    //} else { txt1 = "(" + d.ticker + ") "; }
    ...
    sliTag.select("#cSliderCancel").on('click', function () { that.preBuiltService.selCompany.next(undefined); });
    ...
    var medVel: any = d3.format(".1f")(d.scores*100);
    sliTag.select('.sRect1').style("fill", function () {
      return that.cl(medVel + 1);
    })
    ...
    //sliTag.select('.sRect1').style("fill", that.cl(medVel + 1))
    //sliTag.select('.sRect').attr("stroke-width", 1).style("stroke", that.cl(medVel + 1));
