...
@Component({
  selector: 'app-dashboard-centercircle-spinner',
  templateUrl: './dashboard-centercircle-spinner.component.html',
  styleUrl: './dashboard-centercircle-spinner.component.scss'
})
export class DashboardCentercircleComponentSpinner implements OnInit {
  @Output() triggerRuletReset = new EventEmitter();
  radius = 150;
  glbSearch = new FormControl('');
  filteredOptions: any = [];
  showSearchLoader: boolean = false;
  constructor(public sharedData: SharedDataService, private dataService: DataService, private gloSrcService: GlobalSearchService,public dialog: MatDialog,
    private toastr: ToastrService, private preBuiltService: PrebuildIndexService) {
  ...
  ruletReset() { this.triggerRuletReset.emit(true); };
  STW_Pricecurrency = '$';
  ...
  private _filter(value: any) {
    ...
    if (isNotNullOrUndefined(value) && value != "" && value.trim() !="") {
    ...
    } else { this.filteredOptions = []; this.showSearchLoader = false; }
  }
  ...
  getSearchData(srcTxt: string) {
    return new Promise((resolve, reject) => {
      this.dataService.GlobalSearch(srcTxt).pipe(first()).subscribe((res: any) => { resolve(res) });
    });
  }
  ...
  ngOnInit(): void {
    var that = this;
    var selResData = that.sharedData.selResData.subscribe(res => {
      if (res.length > 0) {
        /* ilter S&P 500 for dashboard show data */
        //console.log(res)
        var data: any = [...res].filter(x => x.indexName == "S&P 500").sort((x: any, y: any) => d3.ascending(x.company, y.company));
        //var data: any = [...res].sort((x: any, y: any) => d3.ascending(x.scores, y.scores));
        data.forEach(function (d: any, i: any) { d.cx = ((i * 360 / data.length) - 90); return d });
        that.createCenterCicle(data);
      }
    });
    this.subscriptions.add(selResData)
  }
  ////getData(res:any) {
  ////const dbScore = res.sort((x:any, y:any) => d3.ascending(x.scores, y.scores));
  ////const dbGICS = res.sort((x:any, y:any) => d3.ascending(x.scores, y.scores));

  ////// dbScore = dbGICS.filter((d, i) => {
  //////    let val = d.scores * 100
  //////    return val < 55 && val > 40
  ////// });
  ////// dbGICS = dbScore;

  ////const selreslen = dbScore.length;
  ////dbScore.forEach((d:any, i:any) => {
  ////  d.countrygroup = d.indexName.indexOf('Europe') > -1 ? 'Europe' : d.country;
  ////  d.US = (d.country == 'USA') ? 'USA' : '';
  ////  d.score = d.scores * 100;
  ////  d.deg = d.score;
  ////  d.indname = this.findIndName(dbGICS, d.industry);
  ////  d.industry = `${d.industry}`;
  ////  d.companyName = d.companyName != null ? this.transformToTitleCase(d.companyName.trim()) : '';
  ...
  ////  /* d.fillColor = "#c3c3c3"; */
  ////  d.fillColor = '#6b7681';
  ////  //const flt = IndexOrder.filter((x) => x.index == d.indexName);
  ////  //d.sortOrder = flt.length > 0 ? flt[0].order : null;
  ////  return d;
  ////});
  ////  this.createCenterCicle(dbScore);
  ////}
 ...
 findIndName(dta:any, code:any) {
  ...
  selResData: any = [];
  createCenterCicle(data:any) {
    var that = this;
    that.selResData = data;
    try {
      if (data.length > 0) {
        that.creatGradienArc(0, 100, [...data], '#excluTempCircleS');
        that.CreateComps('#excluTempCircleS', [...data], "lvlTemp1");
        this.fillCompetives([...data]);
      }
    } catch (e) { }
  }
  fillCompetives(dta: any[], animate = true) {
    let that = this;
    var gs = null;
    let data = dta;

    d3.select("#TempcrlChartS").select("#gCompetitive").remove();
    d3.select("#TempcrlChartS").select("#TempchartCompetitiveRect").remove();
    gs = d3.select("#TempcrlChartS").append("g").attr("id", "gCompetitive");
    d3.select("#TempcrlChartS").append("g").attr("id", "TempchartCompetitiveRect");
    ...
    //console.log('fillCompetives', data);
    ...
    ggs.append("text").attr("x", function (d) { return that.sharedData.txtx1(d) }).attr("id", "gcStxt")
      .style("fill", function(d):any { if (dta.filter((x: any) => x.isin == d.isin).length == 0) { return "var(--leftSideText-B)"; } else { return "var(--leftSideText-B)"; } })
      .attr("transform", function(d):any { if ((d.cx) > 90) { return "rotate(180)"; } })
      .style("opacity", 0)
      .style("text-anchor", function (d): any { if ((d.cx) > 90) { return "end" } })
      .attr("class", " avbold").text(function(d) { return d3.format(",.1f")(d.scores * 100) + "%" });
    ...
    ggs.append("text")
      ...
      .transition()
      //.delay(function (d, i) {
      //  if (animate) { return i * 5; } else { return 0; }
      //})
      .text(function (d: any) {
        if ((d.cx) > 90) {
          let txt = d.companyName.trim() + "...";
          let resvtxt = "";
          var cnt = txt.length;
          ...
          return d['companyName'].slice(0, (tolen < 0 ? 1 : tolen)).trim() + dottxt;
        }
        else {
          let txt = d.companyName.trim() + "...";
          let resvtxt1 = "";
    ...
    //    that.fillCompClick(d);
    //  });

    //if (data.length > fillMaxlen) { that.fillCompetivesRect(dta); }
    //else { d3.select("#TempchartCompetitiveRect").selectAll("g").remove(); };

    if (dta.length < 100) { d3.select('#exclusionTempchartS').select('#gCompetitive').selectAll('g').style('opacity', '0.6').style('cursor', 'pointer'); }
    else { d3.select('#exclusionTempchartS').select('#gCompetitive').selectAll('g').style('opacity', '0.4').style('cursor', 'pointer'); }

  }
  creatGradienArc(sMin = 0, sMax = 100, datas: any, masterID: any) {
    let that = this;
    if (sMin == null) { sMin = 0; }
    ...
    gArc = d3.select(masterID).select("#TempcrlChartS").append("g").attr("id", "gradArc");
    ...
    //var gC100 = d3.scaleLinear<any>().domain([0, 25, 50, 75, 100]).range(["var(--iconColor)", "var(--iconColor)", "var(--iconColor)", "var(--iconColor)", "var(--iconColor)"]);
    var gC100 = d3.scaleLinear<any>().domain([0, 25, 50, 75, 100]).range(["#7e7e85", "#7e7e85", "#7e7e85", "#7e7e85", "#7e7e85"]);
    ...
    // //console.log('creatGradienArc', Data1);
    ...
    // 360 degrees
    d3.range(4).forEach(function (d: any, i: any) {
      // convert to radians
    ...
    gArc.selectAll('.arc')
      ...
      .attr('d', arc)
      .attr('stroke', 'none')
      .attr('fill', function (d: any, i: any) { return that.clipPathCheck(path + 'linearColors' + i, '', masterID); });
    ...
    gArc.append('path')
      ...
      .style('transform', 'scale(8.35) translate(18px, -18px) rotateY(180deg)')
    ...
    var svg = d3.select("#TempcrlChartS").append('g').attr('id', 'holder').append('g').attr('id', 'rotateHolder').style('transform', 'rotate(360deg)');
    var handles = svg.append('g').attr('id', 'handles')
      .attr('transform', 'translate(' + (0) + ',' + (2) + ')').attr("class", "sliderDisp").style("display", "block");
    handles.append('g').attr('transform', ('translate(0,' + (radius) * -1 + ')')).append('circle').attr('r', handleRadius).style('stroke', '#d0d4dc')
      .style('stroke-width', handleStrokeWidth).attr('cursor', 'pointer').style('fill', "#d0d4dc").style('stroke-opacity', 1)
    that.startStockDataCalls();
  }

  CreateComps(masterID: any, dta: any, lvl: any) {
    let that = this;
    var compLst;
    var oSvg = d3.select(masterID).select("#TempcrlChartS");
    ...
    //var fillMaxlen = 500;
    //var sublen = parseInt((dta.length / fillMaxlen).toFixed(0));
    //var sdata = [];
    //if (dta.length > fillMaxlen) { sdata = dta.filter(function (d: any, i: number) { if (i == 0 || (i % sublen) == 0) { return d; } }); }
    //else { sdata = dta; }
    
    var compg = compC.selectAll<SVGGElement, any>("g").data(dta).enter().append("g").attr("class", 'comp')
      .attr("transform", function (d: any) { return "rotate(" + (d.cx) + ")"; })
      .attr("name", function (d: any) { return d.isin + "_" + d.indexName.replace(/ /g, '_') })
      .on("mouseover", function (d: any) { d3.select(d).select(".crect").classed("onrect", true); })
      .style("display", "block")
      .on("mouseout", function (d: any) { d3.select(d).select(".crect").classed("onrect", false); });
    ...
    compg.append("rect")
      .attr("height", "1px")
      .attr("class", function () { if (dta.length > 600) { return ("crect rectOpa "); } else { return ("crect "); } })
    ...
    if (isNotNullOrUndefined(dta) && dta.length > 0) {
  ...
  @ViewChild('glbSearchBox') glbSearchBox!: ElementRef<HTMLInputElement>;
  speackClick() {
    this.glbSearch.reset();
    try {     
      this.toastr.info('Click/Tap the record icon, say the Space, and speak your command', '', { timeOut: 4000 });
     ...
     recognition.onend = (e: any) => {
       if (final.length>0 && final.toLowerCase().indexOf('space') > -1) {
         var txt: any = final.toLowerCase().split('space');
         var src: string = (txt.length > 1) ? txt[1].trim() : '';
         this.glbSearch.setValue(src);
         this.glbSearchBox.nativeElement.focus();
       } else { }
       recognition.stop();
     };
  ...
  fillCompetivesRect(data: any) {
    var that = this;
    var gs: any;
    d3.select("#TempchartCompetitiveRect").selectAll("g").remove();
    d3.select("#TempchartCompetitiveRect").raise();
    gs = d3.select("#TempchartCompetitiveRect");
    // console.log(data)
  ...
  gfillCompMouseover(gs: any, d: any, dta: any, ev: any) {
    ...
    ggs.append("text").attr("x", that.sharedData.txtx1(d)).attr("id", "gcStxt")
      ...
      .attr("class", " avbold").text(function () { return d3.format(",.1f")(d.scores * 100) + "%" });
    ...
    ggs.on("click", function () { that.fillCompClick(d); });
   
  }

  gfillCompMouseoverRotate(d: any, dta: any) {
    var that = this;
   
    var gs = d3.select("#TempchartCompetitiveRect");
    gs.selectAll(".gfillCompMouseover").remove();
    var ggs = gs.append("g").attr("class", "gfillCompMouseover")
      .style("cursor", "pointer").style("font-size", "10px").style("font-family", 'var(--ff-bold)')
      .attr("transform", function () {
        if (d.cx <= 90) { return "rotate(" + (d.cx + 1.0) + ")"; } else { return "rotate(" + (d.cx - 1.0) + ")"; }
      });
    //d3.select(ev).raise();
    var colorVal = that.checkHandleColor(dta, d);
    ggs.append("text").attr("x", that.sharedData.txtx1(d)).attr("id", "gcStxt")
      //.style("fill", function (): any { if (dta.filter((x: any) => x.isin == d.isin).length == 0) { return "var(--leftSideText-B)"; } else { return "var(--leftSideText-B)"; } })
      .style("fill", colorVal)
      .attr("transform", function (): any { if ((d.cx) > 90) { return "rotate(180)"; } }).style("text-anchor", function():any {if ((d.cx) > 90) { return "end" }})
      .attr("class", " avbold").text(function () { return d3.format(",.1f")(d.scores * 100) + "%" });
  }
  currentStockIndex = 10;
  currentCircleRotate = 360;
  currentCircleRotateClock = 0;
  intervalId: any;
  currentValSpinner: any;
  showSpinBtn: boolean = false;
currentDelay = 1; // Initial high speed
minDelay = 10; // Minimum delay (slower speed)
maxDelay = 1000; // Maximum delay (fastest speed)
maxDelayTimeout = 10000; // Maximum delay (fastest speed)
  ...
  startStockDataCalls_d() {
    var that = this;
    let rotateDuration = 10; // Initial duration of rotation
    const maxRotateDuration = 1000; // Maximum duration to slow down the rotation
    const rotateIncrement = 10; // Increment step for increasing duration

    const updateRotation = () => {
      if (!this.showSpinBtn) {
        d3.select('#rotateHolder')
          .transition()
          .duration(rotateDuration)
          .ease(d3.easeLinear)
          .style('transform', 'rotate(' + this.currentCircleRotate + 'deg)')
          .on('end', () => {
            this.currentCircleRotate--;
            if (this.currentCircleRotate < 0) {
              this.currentCircleRotate = 360; // Reset when completing a full rotation
            }

            d3.select('#TempcrlChartS')
              .select('.compLstClvlTemp1')
              .transition()
              .duration(rotateDuration)
              .ease(d3.easeLinear)
              .style('transform', 'rotate(' + this.currentCircleRotate + 'deg)');

            var findCX = this.currentCircleRotate - 90;
            var dst = [...that.selResData].filter((x: any) => x.cx >= findCX);

            if (dst.length > 0) {
              that.callStockData(dst[0]);
              that.currentValSpinner = dst[0];
            }

            // Increase the duration of the transition gradually
            if (rotateDuration < maxRotateDuration) {
              rotateDuration += rotateIncrement;
            }

            // Call the function recursively for continuous rotation
            updateRotation();
          });
      }
    };
    ...
    // Stop the rotation after 12 seconds
    setTimeout(() => {
      this.showSpinBtn = true;
      that.createSliderOnce(that.currentValSpinner);
    }, 12000);
  }
  startStockDataCalls() {
    var that = this;
    ...
    const updateInterval = () => {
      ...
      if (!this.showSpinBtn) {
        // Start the new interval with the updated delay
        that.intervalId = setInterval(() => {
          this.currentCircleRotate--;
          this.currentCircleRotateClock++;
          ...
          d3.select('#rotateHolder')
            ...
            .style('transform', 'rotate(' + this.currentCircleRotate + 'deg)');
          d3.select('#TempcrlChartS')
            .select('.compLstlvlTemp1')
            .transition()
            //.duration(5000)
            .ease(d3.easeCubicOut)
            .style('opacity', .75)
            .style('transform', 'rotate(' + this.currentCircleRotateClock + 'deg)')
            .on('end', () => {
              // Ensure the wheel remains in a valid rotational position after the animation
              d3.select('#TempcrlChartS').select('.compLstlvlTemp1').style("opacity", 1)
                //.style('transform', `rotate(${this.currentCircleRotate % 360}deg)`); // Keep it between 0-360 degrees
            });

          if (this.currentCircleRotate === 0) {
            this.currentCircleRotate = 360;
            this.currentCircleRotateClock = 0;
          }
          var findCX = this.currentCircleRotate - 90;
          var dst = [...that.selResData].filter((x: any) => x.cx >= findCX);
      ...
      if (!this.showSpinBtn) {
        setTimeout(() => {
          if (that.currentDelay < that.maxDelay) {
            that.currentDelay += that.currentDelay + 3; 
            updateInterval();
          }
        }, 2000);
      }
    ...
    setTimeout(() => {
      that.showSpinBtn = true;
      //console.log('currentValSpinner', this.currentValSpinner)
      var colorVal = that.checkHandleColor([...that.selResData], that.currentValSpinner);
      var d: any = that.currentValSpinner;
      var Cname: string = (isNotNullOrUndefined(d.companyName)) ? d.companyName : "";
      let txt = Cname + " (" + d.ticker + ")";
      let txt1 = d3.format(".1f")(d.score) + "%";
      if (txt1.length > 24) { txt1 = "(" + d.ticker + ") " + d3.format(".1f")(d.score) + "%"; }
      if ((Cname.length) > 14) { txt = Cname.slice(0, 12).trim() + "..." + " (" + d.ticker + ")"; }
     
      d3.select("#circleSelectedName").select(".CText").text(txt)
      that.HFscore = txt1;
      that.STW_Pricecurrency = d['currency'] + ' ' + d3.format(",.1f")(d['price']);
      that.impRevValue = d3.format(".1f")(d['impRev'] * 100);
      //d3.select("#circleSelectedName").select(".MText").text(txt1).style("fill", colorVal); 
      d3.select('#TempcrlChartS').select('.compLstlvlTemp1').style("opacity", 1);
      d3.select('#circleLogo').style("opacity", 0);
      d3.select('#circleSelectedName')// Start with the group invisible
        .transition() // Start the transition
        .duration(2000) // 2 seconds transition time
        .ease(d3.easeSinIn) // Elastic easing for a bouncy effect
        .style('opacity', 1); // Fade in to full opacity

      d3.select('#fil_STW_Price')// Start with the group invisible
        .transition() // Start the transition
        .duration(2000) // 2 seconds transition time
        .ease(d3.easeSinIn) // Elastic easing for a bouncy effect
        .style('opacity', 1); // Fade in to full opacity
    ...
    //if (that.currentDelay == 1) {
    //  d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animateS5", true);
    //  d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animateS10", false);
    //  d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animate", false);
    //} else if (that.currentDelay > 15) {
    //  if (that.currentDelay > 75) {
    //    d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animateS10", false);
    //    d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animateS5", false);
    //    d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animate", true);

    //  } else {
    //    d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animateS5", false);
    //    d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animate", false);
    //    d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animateS10", true);
    //  }
    //}
    // d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animate", true);
    //that.intervalId = setInterval(() => {
    //  this.currentCircleRotate--;
    //  d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animate", true); 
    //  d3.select('#rotateHolder').style('transform', 'rotate(' + this.currentCircleRotate + 'deg)');
    //  if (this.currentCircleRotate == 0) {
    //    this.currentCircleRotate = 360
    //  };
    //  var findCX = that.currentCircleRotate - 90;
    //  var dst = [...that.selResData].filter((x: any) => x.cx >= findCX);
    // // console.log(dst);
    //  if (dst.length > 0) {
    //    that.callStockData(dst[0]);
    ...
    //  clearInterval(this.intervalId);
    //  d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animate", false);
    //  this.createSliderOnce(this.currentValSpinner);
    //  this.showSpinBtn = true;
    //}, 10000)
  }
  spinAgain() {
    clearInterval(this.intervalId);
    //d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("S20", false);
    //d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("S10", false);
    //d3.select('#TempcrlChartS').select('.compLstClvlTemp1').classed("animateS5", false);
    d3.selectAll("#cSlider").remove();
   
    d3.select('#circleSelectedName').style('opacity', 0) // Start with the group invisible
    d3.select('#fil_STW_Price').style('opacity', 0) // Start with the group invisible
    d3.select('#circleLogo').style("opacity", 1);
    this.currentDelay = 1;
    this.showSpinBtn = false;
    this.startStockDataCalls();
  }
  createSliderOnce(d:any) {
    this.creatClockSlider([...this.selResData], d);
  }
  callStockData(stock: any) {
    this.creatClockSlider([...this.selResData], stock);
    //this.gfillCompMouseoverRotate(stock, this.selResData)
    //console.log("Stock Data:", stock);
    // You can implement further logic for each call here
  }
  ...
  creatClockSlider(dta: any, selComp: any) {
    ...
    var r: any = 150;
    d3.selectAll("#cSlider").remove();
    var g = d3.select("#TempcrlChartS").append("g").attr("id", "cSlider").attr('transform', 'rotate(' + (-90) + ')').on('mousedown', onDown).on("touchstart", onDown)
      .on("pointerenter", function (eve) { if (d3.selectAll("#gCompMOver").style("display") != 'none') { d3.selectAll("#gCompMOver").style("display", 'none') } });

    g.append("rect").attr("class", "sRect1").attr("x", r).attr("y", -1.2).attr("height", 3).attr("width", 50).attr("fill", "#767676");

    g.append("rect").attr("class", "sRect").attr("rx", 20).attr("ry", 20).attr("x", + r + (25)).attr("height", "42").attr("width", "125").style("display", "none");

    g.append("rect").attr("class", "sRectCircle").attr("rx", 20).attr("ry", 20).attr("x", + r + (27)).attr("height", "36").attr("width", "36");
    ...
    g.append("text").attr("class", "sTextT").attr("x", + r + (40)).attr("y", 10).style("font-family", 'var(--ff-medium)').style("font-size", "9px")
      .style("display", "none").text("0.00");

    g.append("text").attr("class", "sText1").attr("x", + r + (11)).attr("y", 3).style("display", "none")
      .style("font-size", "8px").style("text-anchor", "middle").attr("fill", "var(--prim-button-color)").style("font-family", 'var(--ff-semibold)').text("0.00");
  ...
  SetClockHandle(data: any, d: any) {
    ...
    var Cname: string = (isNotNullOrUndefined(d.companyName)) ? d.companyName : "";
    let txt = Cname;  
    let txtTicker = "(" + d.ticker + ")";  
    //let txt1 = "(" + d.ticker + ") " + d3.format(".1f")(d.score) + "%";
    let txt1 = d3.format(".1f")(d.score) + "%";
    //if (txt1.length > 24) { txt1 = "(" + d.ticker + ") " + d3.format(".1f")(d.score) + "%"; }
    if ((Cname.length) > 12) { txt = Cname.slice(0, 12).trim() + "..."; }
    let val = d.cx;
    ...
    d3.select("#cSlider").select(".sText").text(txt).style("display", function () { return txt == null ? "none" : "block"; })
      .attr("x", function () {
        ...
        return d.cx > 90 ? ((parseFloat(r) + txtlen + 77) * -1) : "220";
      })
      .attr("transform", function () { return d.cx > 90 ? "rotate(180)" : null; }).attr("fill", "var(--leftSideText-B)")
    d3.select("#cSlider").select(".sTextT").text(txtTicker).style("display", function () { return txtTicker == null ? "none" : "block"; })
      .attr("x", function () {
        var txtlen1 = that.sharedData.measureText(txt1, 9);
        return d.cx > 90 ? ((parseFloat(r) + txtlen1 + 77) * -1) : "220";
      }).style("font-size", '10px')
      .attr("transform", function () { return d.cx > 90 ? "rotate(180)" : null; }).attr("fill", "var(--leftSideText-B)");
    d3.select("#cSlider").select(".sText1").text(txt1).style("display", function () { return txt1 == null ? "none" : "block"; })
      .attr("x", function () {
        var txtlen1 = that.sharedData.measureText(txt1, 9);
        return d.cx > 90 ? ((parseFloat(r) + txtlen1 + 19) * -1) : "195";
      }).style("font-size", '10px')
      .style("fill", function () { return d.score > 25 && d.score < 60 ? "var(--primary-color)" : "var(--leftSideText-B)"; })
      .attr("transform", function () { return d.cx > 90 ? "rotate(180)" : null; }).attr("fill", "var(--leftSideText-B)");
    ...
    d3.select("#cSlider").select(".sRectCircle").style("display", function () { return txt == null ? "none" : "block"; })
      .attr("y", -(bboxH / 2)+2);
    ...
    d3.select("#cSlider").select("#cSliderCancel").on('click', function () { that.removeHandle(); });
    ...
    d3.select("#cSlider").select(".sTextReverse").attr("fill", "#fff")
      .style("display", function () { return txt == null ? "none" : "block"; })
      .attr("x", function (): any {
    ...
    var colorVal = that.checkHandleColor(data, d);
    ...
    d3.select('#cSlider').select('.sRectCircle').style("fill", colorVal);   
  }
  ...
  removeHandle() { d3.selectAll("#cSlider").remove(); }
  openImpliedRevenue() {
    var selcomp: any = this.currentValSpinner;
    if (isNotNullOrUndefined(selcomp) && isNotNullOrUndefined(selcomp['isin'])) {
      var newSel: any = [...this.sharedData._selResData].filter((x: any) => x.isin == selcomp['isin']);
      if (newSel.length > 0) {
        try {
